...
static void do_bpf_send_signal(struct irq_work *entry)
{
	...
	if (work->is_siginfo) {
		clear_siginfo(&info);
		info.si_signo = work->sig;
		info.si_errno = 0;
		info.si_code = SI_KERNEL;
		info.si_pid = 0;
		info.si_uid = 0;
		info.si_value.sival_int = work->value;
		group_send_sig_info(work->sig, &info, work->task, work->type);
		return;
	}
...
__bpf_kfunc int bpf_send_signal_remote(struct task_struct *task, int sig, enum pid_type type, int value)
{
	struct send_signal_irq_work *work = NULL;
	kernel_siginfo_t info;
