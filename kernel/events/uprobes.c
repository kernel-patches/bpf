...
static void hprobe_init_leased(struct hprobe *hprobe, struct uprobe *uprobe, struct srcu_ctr __percpu *srcu_ctr)
{
	WARN_ON(!uprobe);
	hprobe->state = HPROBE_LEASED;
	hprobe->uprobe = uprobe;
	hprobe->srcu_ctr = srcu_ctr;
}
			      ...
			      struct return_instance *ri)
{
	...
	hprobe_init_leased(&ri->hprobe, uprobe, srcu_ctr);
