...
walk_cfg:
	...
	for (i = 0; i < insn_cnt; i++) {
		...
		if (insn_state[i] != EXPLORED) {
			 ...
			 * expected to have some: the compiler emits a
			 * continuation after a throw it knows never returns,
			 * and bpf_unwind_resume() ends a landing pad, so the
			 * rest of the pad's basic block is stranded. Nothing
			 * branches to either, and refusing the program over
			 * code its own frontend had no way not to emit would
