...
/*
 * Caller must clear owner->storage and uncharge memory if memory charging is used.
 *
 * Since smaps associated with selems may already be gone, mem_uncharge() or owner_storage()
 * cannot be called in this function. Let the caller do it instead. It is safe for the caller
 * to clear owner_storage without taking local_storage->lock as bpf_local_storage_map_free()
 * does not free local_storage and no BPF program should be running and free the local storage.
 */
u32 bpf_local_storage_destroy(struct bpf_local_storage *local_storage)
{
	...
	/*
	 * reuse_now cannot be true since:
	 * 1. The smap holding the bpf_mem_alloc may already be gone
	 * 2. bpf_local_storage_map_free() needs selem->local_storage to be protected by RCU
	 */
	bpf_selem_free_list(&free_selem_list, false);

	bpf_local_storage_free(local_storage, storage_smap, bpf_ma, false);
