...
static inline void __bpf_generic_vlog(struct bpf_generic_log *log, const char *fmt,
				      va_list args)
{
	unsigned int n;
...
__printf(2, 3) void bpf_generic_vlog(struct bpf_generic_log *log,
				     const char *fmt, ...)
{
	va_list args;
	...
	va_start(args, fmt);
	__bpf_generic_vlog(log, fmt, args);
	va_end(args);
}
EXPORT_SYMBOL_GPL(bpf_generic_vlog);
