 ...
 * the LRU list itself. It must be initialized while holding the LRU lock
 * to avoid a race where a popped LRU node is evicted and removed from the
 * hash map with an uninitialized hash value.
 */
