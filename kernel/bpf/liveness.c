...
bpf_insn_successors(struct bpf_verifier_env *env, u32 idx)
{
	...
	/* pre-allocated array of size up to 3; reset cnt, as it may have been used already */
	succ = env->succ;
	succ->cnt = 0;
...
bool bpf_stack_slot_alive(struct bpf_verifier_env *env, u32 frameno, u32 half_spi)
{
	...
	for (i = frameno; i < q->curframe; i++) {
		...
		if (bpf_calls_callback(env, callsite)) {
			alive = is_live_before(instance, callsite, rel, half_spi);
		} else {
			alive = is_live_before(instance, callsite + 1, rel, half_spi);
			/*
			 * The instruction after the call is not the only place
			 * control can come back to: a call an exception cleanup
			 * record covers also resumes at that record's landing
			 * pad, and a slot the pad is the only reader of is
			 * alive there and dead everywhere else.
			 */
			if (!alive && unlikely(env->cleanup_info_cnt)) {
				int pad = bpf_cleanup_pad_of_call(env, callsite);

				/*
				 * A record's range and its pad are in one
				 * subprogram, checked at load time, so the pad
				 * is in this instance -- but this indexes an
				 * array with it, so do not take that on trust.
				 */
				if (pad >= 0 &&
				    (u32)pad - instance->subprog_start < instance->insn_cnt)
