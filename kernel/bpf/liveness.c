...
struct func_instance {
	...
	u32 callsites[MAX_CALL_FRAMES]; /* callsites[i] = call insn at frame i (for outer frame lookup) */
					   ...
					   u32 callsite, int subprog)
{
	...
	if (caller)
		memcpy(f->callsites, caller->callsites, sizeof(f->callsites));
	if (depth > 0)
		f->callsites[depth - 1] = callsite;
...
static int mark_stack_write(struct func_instance *instance, u32 frame, u32 insn_idx, spis_t mask)
{
	...
	if (instance->must_write_initialized)
		masks->must_write = spis_and(masks->must_write, mask);
	else
...
static char *fmt_subprog(struct bpf_verifier_env *env, int subprog)
{
	snprintf(env->tmp_str_buf, sizeof(env->tmp_str_buf),
		 "subprog#%d %s", subprog, env->subprog_info[subprog].name);
...
static char *fmt_spis_mask(struct bpf_verifier_env *env, int frame, bool first, spis_t spis)
{
	...
	for (spi = 0; spi < STACK_SLOTS / 2 && buf_sz > 0; spi++) {
		...
		/* half-spi, print individually */
		if (!lo || !hi) {
			n = snprintf(buf, buf_sz, "%sfp%d%d%s",
				     first ? "" : " ",
				     frame,
				     -(spi + 1) * BPF_REG_SIZE +
					(lo ? BPF_HALF_REG_SIZE : 0),
				     "h");
			first = false;
			buf += n;
			buf_sz -= n;
		...
		/* full spi, peek 4 bits ahead to check for 3+ consecutive */
		if (spi + 2 < STACK_SLOTS / 2 &&
			...
			n = snprintf(buf, buf_sz, "%sfp%d%d..%d",
				     first ? "" : " ",
				     frame,
				     -(run_start + 1) * BPF_REG_SIZE,
				     -(spi + 1) * BPF_REG_SIZE);
		} else {
			n = snprintf(buf, buf_sz, "%sfp%d%d",
				     first ? "" : " ",
				     frame,
				     -(spi + 1) * BPF_REG_SIZE);
		}
		...
		if (src_is_local_fp && BPF_MODE(insn->code) == BPF_MEM && sz == 8) {
			*dst = fill_from_stack(insn, at_out, insn->src_reg, at_stack_out, depth);
			...
			struct arg_track *parent_stack =
				env->callsite_at_stack[instance->callsites[src->frame]];
 ...
 *
 */
static void record_stack_access_off(struct func_instance *instance, s64 fp_off,
				    s64 access_bytes, u32 frame, u32 insn_idx)
{
	...
	if (access_bytes > 0) {
		...
		if (slot_lo <= slot_hi) {
			mask = SPIS_ZERO;
			spis_or_range(&mask, slot_lo, slot_hi);
			mark_stack_write(instance, frame, insn_idx, mask);
		}
	}
}
 ...
 */
static void record_stack_access(struct func_instance *instance,
				...
				s64 access_bytes, u32 frame, u32 insn_idx)
{
	...
	for (i = 0; i < arg->off_cnt; i++)
		record_stack_access_off(instance, arg->off[i], access_bytes, frame, insn_idx);
}
				     ...
				     struct arg_track *at, int insn_idx)
{
	...
	if (ptr->frame >= 0 && ptr->frame <= depth) {
		record_stack_access(instance, ptr, sz, ptr->frame, insn_idx);
			       ...
			       int insn_idx)
{
	...
	for (r = BPF_REG_1; r < BPF_REG_1 + num_params; r++) {
		...
		if (frame >= 0 && frame <= depth) {
			record_stack_access(instance, &at[r], bytes, frame, insn_idx);
...
redo:
	...
	for (p = po_end - 1; p >= po_start; p--) {
		...
		arg_track_xfer(env, insn, idx, at_out, at_stack_out, instance);
			   ...
			   struct func_instance *instance)
{
	...
	err = compute_subprog_args(env, &info[subprog], entry_args, instance);
	if (err)
		return err;
	...
	for (int p = po_start; p < po_end; p++) {
		...
		err = analyze_subprog(env, callee_args, info, callee_instance);
		if (err)
			return err;
...
int bpf_compute_subprog_arg_access(struct bpf_verifier_env *env)
{
	...
	err = analyze_subprog(env, NULL, info, instance);
	if (err)
		goto out;
