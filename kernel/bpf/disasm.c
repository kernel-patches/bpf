...
const char *const bpf_movs_string[4] = {
	[0] = "(s8)",
	[1] = "(s16)",
	[3] = "(s32)",
};
...
static const char *const bpf_lds_string[] = {
	[BPF_W >> 3]  = "s32",
	[BPF_H >> 3]  = "s16",
	[BPF_B >> 3]  = "s8",
};
...
static bool is_sdiv_smod(const struct bpf_insn *insn) {
	...
	return (op == BPF_DIV || op == BPF_MOD) && off == 1;
}
		    ...
		    bool allow_ptr_leaks)
{
	...
	if (class == BPF_ALU || class == BPF_ALU64) {
		...
		if (BPF_MODE(insn->code) != BPF_MEM && BPF_MODE(insn->code) != BPF_MEMS) {
