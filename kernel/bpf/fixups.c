				 ...
				 struct bpf_insn *original_insn)
{
	...
	for (i = off; i < off + cnt - 1; i++) {
		...
		/*
		 * Everything a patchlet puts in place of an instruction that
		 * only ran while unwinding does too, wherever in the patchlet
		 * the instruction it replaced ended up.
		 */
...
/*
 * The instructions of @sub that only run while unwinding, as a bitmap for the
 * JIT: a landing pad runs on the bpf_throw() walker's stack, so a JIT that
 * addresses the BPF frame through the stack pointer has to address these
 * through the frame pointer instead.
 */
...
/*
 * A program the JIT compiles as a single function gets its table here rather
 * than from jit_subprogs(), and later: constant blinding still has a turn at
 * it, and it moves every instruction past the first one it rewrites.
 *
 * jit_subprogs() is what handles everything else, so bail on a program it took
 * -- on subprog_cnt, the same test bpf_jit_subprogs() itself makes, and on the
 * table being there already, so that a second call cannot leak the first one's
 * arrays.
 */
int bpf_cleanup_attach_main_prog(struct bpf_verifier_env *env, struct bpf_prog *prog)
{
	if (!env || env->subprog_cnt > 1 || prog->aux->exc)
		return 0;
