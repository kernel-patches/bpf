				...
				enum cgroup_bpf_attach_type atype)
{
	...
	if (atype == CG_BPF_CGROUP_INET_EGRESS) {
				      ...
				      u32 *flags)
{
	...
	cgrp = sock_cgroup_ptr(&sk->sk_cgrp_data);
	ret = BPF_PROG_RUN_ARRAY_FLAGS(cgrp->bpf.effective[atype], &ctx,
				       BPF_PROG_RUN, flags);
				       ...
				       int *optlen, char **kernel_optval)
{
	...
	lock_sock(sk);
	ret = BPF_PROG_RUN_ARRAY(cgrp->bpf.effective[CG_BPF_CGROUP_SETSOCKOPT],
				 &ctx, BPF_PROG_RUN);
	release_sock(sk);
				       ...
				       int retval)
{
	...
	lock_sock(sk);
	ret = BPF_PROG_RUN_ARRAY(cgrp->bpf.effective[CG_BPF_CGROUP_GETSOCKOPT],
				 &ctx, BPF_PROG_RUN);
	release_sock(sk);
