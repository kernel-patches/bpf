					     ...
					     enum bpf_attach_type attach_type)
{
	...
	case BPF_PROG_TYPE_KPROBE:
		...
		if (prog->expected_attach_type == BPF_TRACE_UPROBE_MULTI &&
			...
			return -EINVAL;
		if (prog->expected_attach_type == BPF_TRACE_UPROBE_MULTI_SESSION &&
		    attach_type != BPF_TRACE_UPROBE_MULTI_SESSION)
			return -EINVAL;
		if (attach_type != BPF_PERF_EVENT &&
		    attach_type != BPF_TRACE_KPROBE_MULTI &&
		    attach_type != BPF_TRACE_KPROBE_MULTI_SESSION &&
	...
	case BPF_PROG_TYPE_KPROBE:
		if (attr->link_create.attach_type == BPF_PERF_EVENT)
			...
			ret = bpf_kprobe_multi_link_attach(attr, prog);
			 ...
			 attr->link_create.attach_type == BPF_TRACE_UPROBE_MULTI_SESSION)
