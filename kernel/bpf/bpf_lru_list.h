...
struct bpf_lru_node {
	...
	u8 ref;
	/*
	 * Set by bpf_lru_push_free() when its lock acquisition fails. The
	 * flush and shrink paths route flagged nodes to the global FREE list
	 * regardless of del_from_htab(), which can no longer find the elem
	 * because the htab caller detached it before calling push_free.
	 */
	u8 pending_free;
};
