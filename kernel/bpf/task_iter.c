...
again:
	...
	/* Find and validate VMA using per-VMA locks */
	{
		...
		/* Find VMA under RCU, then validate with per-VMA lock.
		 * We release the lock before returning to avoid lock ordering issues.
		 */
		rcu_read_lock();
		...
		/* Validate the VMA with per-VMA lock */
		...
		/* We have a valid VMA - release lock before returning */
		curr_vma = locked_vma;
		vma_end_read(locked_vma);
	}
	   ...
	   bpf_callback_t, callback_fn, void *, callback_ctx, u64, flags)
{
	...
	/* Find and validate VMA under RCU without holding locks during callback.
	 * We cannot hold vm_lock or mmap_lock during the callback because the
	 * callback may call bpf_copy_from_user_task() which needs mmap_lock,
	 * creating a lock ordering violation with the kernel's mmap_lock->vm_lock
	 * dependency.
	 */
	rcu_read_lock();
	...
	/* Validate VMA covers the address while we hold the lock */
	if (vma->vm_start <= start && vma->vm_end > start) {
		/* VMA is valid - release lock before callback to avoid lock ordering issues.
		 * The VMA structure is RCU-protected so it won't be freed, but fields
		 * might change. This is acceptable for BPF tracing/profiling use cases.
		 */
		vma_end_read(vma);
...
__bpf_kfunc struct vm_area_struct *bpf_iter_task_vma_next(struct bpf_iter_task_vma *it)
{
	...
	/* Find next VMA under RCU, then validate it with per-VMA lock.
	 * We release the lock before returning to avoid lock ordering issues
	 * if the BPF program calls bpf_copy_from_user_task().
	 */
	rcu_read_lock();
	...
	/* Validate and lock the VMA we found */
	vma = lock_vma_under_rcu(kit->data->mm, vm_start);
	if (!vma)
		return NULL;

	/* Save end address for next iteration, then release lock */
	kit->data->prev_vm_end = vma->vm_end;
	vma_end_read(vma);

	/* Return VMA without holding locks - RCU protects from being freed */
	return vma;
}
