			      ...
			      bpfptr_t uattr)
{
	...
	for (i = 0; i < nrec; i++) {
		...
		if (rec->landing_pad_off >= rec->begin_off &&
		    rec->landing_pad_off < rec->end_off) {
			verbose(env, "cleanup_info[%u]: landing pad %u inside call-site range\n",
				i, rec->landing_pad_off);
			goto err_free;
		}
	}
