...
/*
 * Whether @i only ever runs with an exception in flight. Kept in
 * insn_aux_data rather than in the scratch above because the JIT needs it
 * too, long after this pass: bpf_patch_insn_data() and
 * bpf_verifier_remove_insns() carry it along with the instruction it is on.
 */
...
/* Add edge @e, which runs into @to, to @to's predecessor list. An instruction
 * has at most two successors, so edge 2*i is i's fall-through and edge 2*i + 1
 * its branch target, and the source of an edge is the edge id over two. Both
 * the heads and the links are one-biased, 0 being the end of a list.
 */
...
/*
 * A landing pad runs on the bpf_throw() walker's stack with nothing of its own
 * frame put back but that frame's registers, so an instruction whose lowering
 * is anchored to the stack pointer acts on the walker's frame instead of on
 * the pad's own. Refuse the two that are:
 *
 *   - a tail call, which unwinds the prologue off the stack pointer before
 *     branching, so it would pop the walker's frame and hand the target a
 *     stack pointer into the middle of it;
 *   - a call that passes an argument on the stack. The outgoing area is
 *     reserved in the frame's prologue and found again by the callee relative
 *     to the stack pointer at the call, and here those are no longer the same
 *     memory.
 *
 * Neither is something a frontend lowering cleanups has any reason to emit --
 * no unwinder tail calls, and a destructor takes one argument.
 *
 * Reading an argument the frame itself was passed on the stack would have the
 * same problem on an arch that finds it through the frame record, but needs no
 * rule here: check_stack_arg_read() already requires every such read to come
 * before the frame's first call, and a pad only ever runs after one.
 */
static int cleanup_check_pad_insn(struct bpf_verifier_env *env, u32 i)
{
	...
	/*
	 * The same thing reached the other way: a kfunc taking a by-value
	 * struct or an __int128 can run out of argument registers without the
	 * program storing anything itself, and then it is the JIT that puts
	 * the rest in the outgoing area.
	 */
	if (bpf_helper_call(insn) || bpf_pseudo_kfunc_call(insn)) {
...
int bpf_prepare_cleanup_exceptions(struct bpf_verifier_env *env)
{
	...
	/*
	 * A device JIT knows nothing about landing pads, and jit_required
	 * below does not reach it: bpf_fixup_call_args() leaves an offloaded
	 * program to bpf_prog_offload_compile(). The pads would never run,
	 * having been accepted on the strength of what they release.
	 */
...
/*
 * Every subprogram of a program that carries a cleanup table spills every BPF
 * callee-saved register in its prologue, even one with no records of its own
 * and one that never throws: a frame's spill holds its caller's registers, and
 * that is what the walker restores before running the caller's pad. The
 * exception callback does not: it runs only once the walk is over, and its
 * prologue reuses the throwing program's outermost frame (the exception
 * boundary's) rather than building one of its own.
 */
...
void bpf_cleanup_fill_native_ranges(struct bpf_prog *prog, u32 *addrs, void *image)
{
	...
	for (i = 0; i < n; i++) {
		...
		/* end_off is exclusive and may be one past the last insn. */
...
/*
 * Does @idx only ever run with an exception in flight? A JIT that addresses a
 * frame through the stack pointer has to address it some other way here: the
 * stack pointer belongs to the bpf_throw() walker, not to the frame the pad
