 ...
 * what the CFG itself depends on. It refuses the two shapes that need no
 * control flow to see -- a program the kernel could not dispatch pads for at
 * all, and one that installs a bpf_exception_cb() as well -- and paints each
 * covered call site with the pad it unwinds to. Landing pads are unreachable
 * in the compiler's CFG, and that mark is the edge bpf_cleanup_pad_of_call()
 * hands bpf_check_cfg() to stop them being reported as dead code.
 *
 ...
 *
 * The refusals here, all of them shapes with no correct answer rather than
 * shapes merely unimplemented:
 *
 ...
 *    call;
 *  - a bpf_unwind_resume() that is not inside the body of a landing pad,
 *    where the bare return both JITs emit for it would leave a live frame
 *    without running its epilogue;
 ...
 *
 * One refusal does not fit either pass, and waits for do_check() instead:
 * bpf_cleanup_check_callback() turns away a subprogram that may unwind at the
 * point a helper is handed it as a callback (bpf_loop() and friends). The
 * helper's own kernel frame sits between the callback and the program, and the
 * walk stops at the first frame that is not BPF, so the exception would never
 * reach a boundary at all. Only do_check() knows a callback is really taken.
 *
...
/*
 * Refuse a subprogram that may unwind, about to be invoked as a callback.
 *
 * bpf_throw() finds the exception boundary by walking the BPF call stack and
 * stops at the first frame that is not BPF. A callback runs below the calling
 * helper's own kernel frame, so the walk would end there: no boundary, no
 * delivery, and no pad above it ever run.
 *
 * Asked at the call, from push_callback_call(), rather than worked out up
 * front from the BPF_PSEUDO_FUNC ld_imm64s that name callbacks. Those name a
 * subprogram without saying it is ever passed to anything, and a program
 * carrying a cleanup table keeps unreachable code until the dead code sweep,
 * so one stranded in a tail the sweep has not reached yet is not evidence of
 * anything at all. Here the callback is real and the code that hands it over
 * is code do_check() reached.
 */
...
static int cleanup_mark_pad_bodies(struct cleanup_ctx *ctx)
{
	...
	/*
	 * Every pad the table names, not only the ones a call site was painted
	 * with: a record whose range holds no call an exception can unwind out
	 * of still names a pad, that pad still contains a resume, and
	 * cleanup_check_resumes() below has to be able to see it. Nothing has
	 * moved an instruction yet, so the table's offsets are still the ones
...
/*
 * Every bpf_unwind_resume() has to sit inside the body of a landing pad.
 *
 * Both JITs turn one into a bare return: the frame is not being returned to,
 * the bpf_throw() walker called the pad and is waiting for it, so no epilogue
 * runs and none is wanted. That is only right where the walker itself handed
 * control over -- a pad, and whatever the pad reaches without leaving its
 * frame, which is exactly in_pad[].
 *
 * Anywhere else the frame really is live and its prologue really did run. In
 * ordinary code the bare return leaves the middle of a frame; in a subprogram
 * a pad calls -- which is allowed, since a pad may call anything that cannot
 * throw -- it returns to whatever that frame's own spill area happens to
 * hold. Neither is something the verifier's walk of the unwind can catch,
 * because to that walk an exception is in flight either way.
 */
...
static void cleanup_paint_pads(struct bpf_verifier_env *env)
{
	...
	for (i = 0; i < env->cleanup_info_cnt; i++) {
		...
		/*
		 * A frontend is free to emit a region around a call the kernel
		 * knows cannot unwind -- a helper or a kfunc, both nounwind
		 * here. Nothing is wrong with the program, but the pad will
		 * never run and is about to be swept as dead code, so say so
		 * rather than let it look like it was dispatched.
		 */
		if (!painted && (env->log.level & BPF_LOG_LEVEL))
			verbose(env,
				"cleanup_info[%u]: range [%u,%u) holds no call an exception can unwind out of; landing pad %u will not run\n",
				i, rec->begin_off, rec->end_off, rec->landing_pad_off);
	}
}
 ...
 * reshaping.
 *
 * That clause is not there for the callback a program installs itself, which
 * a cleanup table cannot be combined with anyway. It is there for the hidden
 * default one that bpf_do_misc_fixups() appends to anything calling
 * bpf_throw() and having none of its own -- which is every program that
 * carries a table, so this is asked and answered on all of them.
 */
...
const struct bpf_cleanup_pad *bpf_cleanup_pad_for_ip(const struct bpf_prog *prog, u64 ip)
{
	...
	u32 l = 0, r = exc ? exc->nr_pads : 0;

	while (l < r) {
		u32 m = l + (r - l) / 2;
		const struct bpf_cleanup_pad *rec = &exc->pads[m];
...
/*
 * Give @aux the exception cleanup state, which every subprogram of a program
 * that carries a table needs whether or not it has records of its own:
 * bpf_cleanup_force_spill() reads it to decide whether this frame's prologue
 * has to spill its caller's BPF callee-saved registers, and the walker wants
 * that of every frame it passes through, not only of the ones with a pad.
 */
int bpf_cleanup_alloc_info(struct bpf_prog_aux *aux)
{
	if (aux->exc)
		return 0;
	aux->exc = kzalloc(sizeof(*aux->exc), GFP_KERNEL_ACCOUNT | __GFP_NOWARN);
...
void bpf_cleanup_fill_native_pads(struct bpf_prog *prog, u32 *addrs, void *image)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (WARN_ON_ONCE(rec->begin_off > prog->len ||
			...
			return;
		exc->pads[i].begin = (u64)(long)image + addrs[rec->begin_off];
		exc->pads[i].end = (u64)(long)image + addrs[rec->end_off];
		exc->pads[i].pad = (u64)(long)image + addrs[rec->landing_pad_off];
	}
	exc->nr_pads = n;
}
...
{
	const struct bpf_exception_info *exc = prog->aux->exc;
	u32 l = 0, r = exc ? exc->nr_throw_at : 0;

	while (l < r) {
		u32 m = l + (r - l) / 2;

		if (idx < exc->throw_at[m])
			r = m;
		else if (idx > exc->throw_at[m])
			l = m + 1;
		else
			return true;
	}
