 ...
 *    (bpf_loop() and friends): the helper's own kernel frame sits between the
 *    callback and the program, and the walk stops at the first frame that is
 *    not BPF, so the exception would never reach a boundary at all;
