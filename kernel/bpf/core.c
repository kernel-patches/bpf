...
static struct bpf_prog *bpf_prog_jit_compile(struct bpf_verifier_env *env, struct bpf_prog *prog)
{
	...
	if (!bpf_prog_need_blind(prog)) {
		 ...
		 * bpf_cleanup_attach_main_prog(). A program that carries one
		 * is jit_required, so returning it unjited fails the load
		 * rather than losing its landing pads.
		 */
		if (bpf_cleanup_attach_main_prog(env, prog))
			return prog;
