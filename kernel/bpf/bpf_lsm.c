 ...
 * @flags: landlock_restrict_self() flags, except LANDLOCK_RESTRICT_SELF_EXECTIME
 *
 * The request accepts a ruleset together with the usual Landlock restriction
 * flags, except LANDLOCK_RESTRICT_SELF_EXECTIME because helper-based
 * restrictions are already staged through @bprm. The staged restrictions are
 * committed only after exec reaches point-of-no-return. This pins the
 * resulting task for later exec transitions. When called from
 * bprm_creds_from_file(), setting LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS_EXECTIME
 * at that point does not retroactively affect privilege-gain decisions already
 * made for the current exec transition.
 */
__bpf_kfunc int
			     ...
			     u32 flags)
{
	...
	err = landlock_restrict_cred(
		bprm->cred, (struct landlock_ruleset *)ruleset, flags, false);
