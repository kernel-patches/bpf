...
static int apply_range_clear_cb(pte_t *pte, unsigned long addr, void *free_pages)
{
	...
	if (free_pages)
		llist_add(&page->pcp_llist, free_pages);
...
static void arena_free_pages(struct bpf_arena *arena, long uaddr, long page_cnt, bool sleepable)
{
	...
	/*
	 * If we got the lock, check and skip already freed regions here itself.
	 */
	if (!ret && !is_range_tree_set(&arena->rt, pgoff, page_cnt)) {
		raw_res_spin_unlock_irqrestore(&arena->spinlock, flags);
		return;
	}

	/* defer if couldn't acquire lock or if context is not sleepable */
	if (ret || !sleepable) {
		/* drop the lock if we succeeded in acquiring it but the context is not sleepable */
		if (!ret)
			raw_res_spin_unlock_irqrestore(&arena->spinlock, flags);
		s = kmalloc_nolock(sizeof(struct arena_free_span), __GFP_ZERO, -1);
		if (!s)
			return;
	...
	/* drop lock as it is not needed anymore */
	raw_res_spin_unlock_irqrestore(&arena->spinlock, flags);
...
static void arena_free_worker(struct work_struct *work)
{
	...
	llist_for_each(pos, list) {
		...
		/* skip already freed spans */
		if (!is_range_tree_set(&arena->rt, pgoff, page_cnt)) {
			/*
			 * set page_cnt and uaddr to 0 so the following loop will detect this and
			 * call kfree_nolock() for this span. Not doing kfree_nolock() here as it
			 * will corrupt the llist.
			 */
			s->page_cnt = 0;
			s->uaddr = 0;
			continue;
	...
	llist_for_each_safe(pos, t, list) {
		/* Skip already freed spans and call kfree_nolock() for them */
		if (!s->page_cnt && !s->uaddr)
			goto skip;
