...
/*
 * Single-word fast path for word-sized keys (4 bytes on 32-bit,
 * 8 bytes on 64-bit). Picked at dispatch when map->key_size ==
 * sizeof(long). Other key sizes fall through to the generic
 * rhashtable jhash2 path.
 *
 * Key buffers are 8-byte aligned: rhtab_elem->data is __aligned(8),
 * and BPF map keys reach us via syscall-copied or BPF-stack buffers
 * that the verifier guarantees aligned for the declared key type.
 * Plain word loads are safe; no get_unaligned() needed.
 */
