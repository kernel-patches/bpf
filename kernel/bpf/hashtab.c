...
static int rhtab_delete_elem(struct bpf_rhtab *rhtab, struct rhtab_elem *elem, void *copy, u64 flags)
{
	int err;
				...
				void *callback_ctx, u64 flags)
{
	 ...
	 * elements visited.
	 */
	while ((elem = rhashtable_next_key(&rhtab->ht, prev_key, rhtab->params))) {
		if (IS_ERR(elem))
			break;
		num_elems++;
