...
static long rhtab_map_delete_elem(struct bpf_map *map, void *key)
{
	...
	/*
	 * Delte path may hold rhashtable bucket spinlock, this check
...
static void rhtab_map_free_internal_structs(struct bpf_map *map)
{
	...
	/*
	 * Bail on resize (-EAGAIN from walk_next or walk_start) to guarantee
	 * forward progress; the final rhtab_free_elem during map_free covers
	 * any element skipped here.
	 */
	rhashtable_walk_enter(&rhtab->ht, &iter);
	rhashtable_walk_start(&iter);

	for (elem = rhashtable_walk_next(&iter);
	     elem && !IS_ERR(elem);
	     elem = rhashtable_walk_next(&iter)) {
		bpf_map_free_internal_structs(map, rhtab_elem_value(elem, map->key_size));

		if (need_resched()) { /* Avoid stalls on large maps */
			rhashtable_walk_stop(&iter);
			cond_resched();
			if (rhashtable_walk_start_check(&iter) == -EAGAIN)
				break;
		}
	}
					       ...
					       bool do_delete)
{
	...
	/*
	 * The cursor is the first key that was not returned by the previous
	 * batch. rhashtable_walk_enter_from() positions iter at it
	 */
	scoped_guard(rcu) {
		rhashtable_walk_enter_from(&rhtab->ht, &iter,
					   ubatch ? cursor : NULL, rhtab->params);
		rhashtable_walk_start(&iter);
	}
	...
	/* Main loop copying to temporary keys and values buffers */
	for (elem = rhashtable_walk_peek(&iter);
	     elem && !IS_ERR(elem) && total < max_count;
	     elem = rhtab_iter_next(&iter)) {
