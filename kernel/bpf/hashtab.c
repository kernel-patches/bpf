 ...
 * in map_gen_lookup(). u32/u64 specializations
...
static struct bpf_insn htab_map_lookup_call(const struct bpf_map *map)
{
	...
	if (map->key_size == sizeof(u32))
		return BPF_EMIT_CALL(__htab_map_lookup_elem_u32);
	if (map->key_size == sizeof(u64))
		return BPF_EMIT_CALL(__htab_map_lookup_elem_u64);
	return BPF_EMIT_CALL(__htab_map_lookup_elem);
}
