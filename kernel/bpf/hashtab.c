...
static __always_inline int rhtab_key_cmp8(struct rhashtable_compare_arg *arg,
					  const void *ptr)
{
	const u64 key1 = *(unsigned long *)arg->key;
	const struct rhtab_elem *key2 = ptr;

	return key1 != *(u64 *)key2->data;
}

static __always_inline u32 rhtab_hashfn8(const void *data, u32 len, u32 seed)
{
	u64 k = *(const u64 *)data;
...
static struct bpf_map *rhtab_map_alloc(union bpf_attr *attr)
{
	...
	if (rhtab->map.key_size == 8) {
		rhtab->params.hashfn = rhtab_hashfn8;
		rhtab->params.obj_cmpfn = rhtab_key_cmp8;
	}
...
static int rhtab_map_alloc_check(union bpf_attr *attr)
{
	...
	/*
	 * map_extra is the initial table size hint (nelem_hint);
	 * upper 32 bits are reserved and must be zero.
	 */
...
/*
 * Static-const params let the rhashtable inline helpers specialize
 * each callsite: hashfn and obj_cmpfn become direct calls (then
 * inlined via __always_inline), and key_len is a compile-time
 * constant.
 */
static const struct rhashtable_params rhtab_params8 = {
	.head_offset = offsetof(struct rhtab_elem, node),
	.key_offset  = offsetof(struct rhtab_elem, data),
	.key_len     = 8,
	.hashfn      = rhtab_hashfn8,
	.obj_cmpfn   = rhtab_key_cmp8,
};
...
{
	if (rhtab->map.key_size == 8)
		return rhashtable_next_key(&rhtab->ht, prev_key, rhtab_params8);
...
static void *rhtab_lookup_elem(struct bpf_map *map, void *key)
{
	...
	if (map->key_size == 8)
		return rhashtable_lookup_likely(&rhtab->ht, key, rhtab_params8);
...
static int rhtab_delete_elem(struct bpf_rhtab *rhtab, struct rhtab_elem *elem)
{
	...
	if (rhtab->map.key_size == 8)
		err = rhashtable_remove_fast(&rhtab->ht, &elem->node, rhtab_params8);
	else
...
static long rhtab_map_delete_elem(struct bpf_map *map, void *key)
{
	...
	/*
	 * Delete path may hold rhashtable bucket spinlock, this check
	 * protects from deadlocks coming from NMI context.
	 * Bucket spinlock disables irqs.
	 */
	if (irqs_disabled())
		return -EBUSY;
				      ...
				      u64 map_flags)
{
	...
	/*
	 * Cancel and zero old special fields (timer/wq/task_work).
	 * copy_map_value() below skips special-field offsets, so the
	 * zeroed state is preserved for the new value.
	 */
	rhtab_check_and_free_fields(rhtab, elem);
...
static long rhtab_map_update_elem(struct bpf_map *map, void *key, void *value, u64 map_flags)
{
	...
	/*
	 * Update path may hold rhashtable bucket spinlock, this check
	 * protects from deadlocks coming from NMI context.
	 * Bucket spinlock disables irqs.
	 */
	if (irqs_disabled())
	...
	if (map->key_size == 8)
		tmp = rhashtable_lookup_get_insert_fast(&rhtab->ht, &elem->node, rhtab_params8);
	else
				...
				void *callback_ctx, u64 flags)
{
	 ...
	 * cond_resched path. While RCU is held, prev_key points directly
	 * into the elem we just visited; only when we drop RCU do we need
	 * a stable copy. Use kmalloc_nolock() since this helper may be
	 * invoked from any BPF context (including NMI/hardirq).
	 */
					       ...
					       bool do_delete)
{
	...
	/*
	 * Disable instrumentation around the delete loop: rhtab_delete_elem()
	 * takes rhashtable bucket locks and a tracing/perf/kprobe BPF program
	 * could otherwise recurse here.
	 */
	bpf_disable_instrumentation();
	rcu_read_lock();
