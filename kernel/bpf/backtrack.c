			    ...
			    bool *changed)
{
	...
	for (;;) {
		...
		for (i = last_idx;;) {
			 ...
			 * before reading it. Entering them here rather than at
			 * the landing pad keeps bt->frame inside the state that
			 * still has those frames, even when the pad is the
			 * first insn of this one.
			 */
			...
			if (hist && hist->unwind_frames) {
				 ...
				 * from a throw or a resume hist->unwind_frames
				 * frames deeper. The walker leaves a constant in
				 * r0 on the way in, so no insn defines it and a
				 * request for it ends here. No insn stands
				 * between the two frames either, so the walk
				 * enters them above, the way BPF_EXIT enters one.
				 */
				bt_clear_reg(bt, BPF_REG_0);
				if (bt_empty(bt))
					return 0;
