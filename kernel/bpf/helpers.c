...
bpf_iter_bits_new(struct bpf_iter_bits *it, const void *unsafe_ptr__ign, u32 nr_bits)
{
	...
	if (nr_bits <= 64) {
		...
		u32 offset = IS_ENABLED(CONFIG_S390) ? sizeof(u64) - size : 0;
	...
	err = bpf_probe_read_kernel_common((char *)(kit->bits + words - 1) + sizeof(u64) - left,
					   left, unsafe_ptr__ign + (words - 1) * sizeof(u64));
	if (err) {
		bpf_mem_free(&bpf_global_ma, kit->bits);
		return err;
	}
