				   ...
				   struct bpf_insn *insn)
{
	...
	if (dst_reg->type == PTR_TO_ARENA || (src_reg && src_reg->type == PTR_TO_ARENA)) {
		...
		if (bpf_insn_is_hmul(insn)) {
			verbose(env, "UHMUL/SHMUL on arena pointers are not supported\n");
			return -EOPNOTSUPP;
		}
...
static int check_alu_fields(struct bpf_verifier_env *env, struct bpf_insn *insn)
{
	...
	case BPF_DIV:
	case BPF_MOD:
		/* UHMUL and SHMUL are only defined for BPF_ALU64. */
		if (opcode == BPF_MUL && class == BPF_ALU &&
		    bpf_is_hmul_variant(insn->off)) {
			verbose(env, "UHMUL/SHMUL not supported on BPF_ALU class\n");
			return -EINVAL;
		}
