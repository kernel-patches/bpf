...
static bool is_task_work_add_kfunc(enum bpf_func_id func_id);
				  ...
				  struct bpf_kfunc_call_arg_meta *meta)
{
	...
	if (map->record->task_work_off != val + reg->off) {
		verbose(env, "off %lld doesn't point to 'struct bpf_task_work' that is at %d\n",
		...
		case KF_ARG_PTR_TO_MAP:
			...
			if (meta->map.ptr && reg->map_ptr->record->task_work_off >= 0) {
				if (meta->map.ptr != reg->map_ptr ||
				    meta->map.uid != reg->map_uid) {
					verbose(env,
						"bpf_task_work pointer in R1 map_uid=%d doesn't match map pointer in R2 map_uid=%d\n",
						meta->map.uid, reg->map_uid);
					return -EINVAL;
				}
			}
