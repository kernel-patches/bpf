...
static int arg_slot_from_argno(argno_t a)
{
	int arg = arg_from_argno(a);

	return arg > 0 ? arg : reg_from_argno(a);
}
		...
		enum bpf_access_type type, struct bpf_call_arg_meta *meta)
{
	...
	bool raw_mode = meta && meta->arg_raw_mem.slot == arg_slot_from_argno(argno);
			  ...
			  int insn_idx)
{
	...
	/* Helper raw memory is recorded while validating its static prototype.
	 * A kfunc argument can become raw memory only after resolving a
	 * scalar-only BTF struct against the register passed by the program.
	 */
	if (meta->btf && arg_type_is_raw_mem(arg_type)) {
		if (meta->arg_raw_mem.slot) {
			verbose(env,
				"kernel function %s has more than one uninitialized memory argument\n",
				meta->func_name);
			return -EINVAL;
		}
		meta->arg_raw_mem.slot = slot + 1;
	}
...
static int mark_raw_mem_initialized(struct bpf_verifier_env *env,
				    struct bpf_call_arg_meta *meta, int insn_idx)
{
	struct bpf_func_state *caller = cur_func(env);
	struct bpf_reg_state *reg;
	...
	if (!meta->arg_raw_mem.size)
		return 0;
	if (WARN_ON_ONCE(!meta->arg_raw_mem.slot))
		return -EFAULT;

	slot = meta->arg_raw_mem.slot - 1;
	reg = get_func_arg_reg(caller, cur_regs(env), slot);
	argno = argno_from_arg(slot + 1);
			     ...
			     int *insn_idx_p)
{
	...
	/* Mark slots with STACK_MISC in case of raw mode. */
	err = mark_raw_mem_initialized(env, &meta, insn_idx);
	if (err)
		return err;
				 ...
				 enum bpf_arg_type *arg_type, u32 *arg_size)
{
	...
	*arg_type = ARG_PTR_TO_MEM | MEM_FIXED_SIZE |
		    (*arg_type & (PTR_MAYBE_NULL | MEM_UNINIT | MEM_WRITE));
