...
/*
 * A landing pad runs in its own frame, entered by an ordinary return:
 * bpf_unwind() rewrote the frame's return address, so the call it is
 * suspended at resumes at the pad instead of at the next instruction. The
 * pad's entry state is therefore the state at that call with the
 * caller-saved registers gone -- the callee's epilogue put r6-r9 and the
 * stack back on the way out.
 */
...
/*
 * bpf_unwind() does not return to the instruction after its call: it rewrote
 * its own return address along with every other frame's. Control resumes at
 * this frame's pad when a record covers the call, and otherwise the frame
 * returns at once -- which its caller already explores as the other side of
 * its own covered call.
 */
	...
	case BPF_JMP32: {
		...
		if (opcode == BPF_CALL) {
			...
			if (bpf_is_unwind_resume_kfunc(insn)) {
				/*
				 * Lowered to 'r0 = 0; exit' by the fixups, so
				 * the frame returns a real zero. Unknown
				 * first: the known-zero helpers keep whatever
				 * type r0 already had, which here is NOT_INIT.
				 */
