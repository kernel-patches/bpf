			 ...
			 struct bpf_verifier_state *state, int *prev_insn_idx)
{
	...
	if (class == BPF_ALU || class == BPF_ALU64) {
		...
		/* check that memory (src_reg + off) is readable, the state of
		 * dst_reg will be updated by this func
		 */
...
process_bpf_exit_full:
			 ...
			 * state when it exits.
			 */
			err = check_resource_leak(
				env, exception_exit, !env->cur_state->curframe,
