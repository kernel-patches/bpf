...
static int check_max_stack_depth(struct bpf_verifier_env *env)
{
	 ...
	 * stack, with that frame's registers put back from a spill area whose
	 * contents arch_bpf_run_cleanup_pad() knows how to read. The x86-64 JIT
	 * addresses a private-stack program's frame through a scratch register
	 * rather than rbp, and no spill area holds it, so a pad there would
	 * address its frame through whatever the kernel left behind. Refuse the
	 * combination everywhere rather than make this arch-conditional;
	 * nothing is lost but an optimization.
	 */
...
static int process_cleanup_resume(struct bpf_verifier_env *env, int *insn_idx)
{
	...
	/*
	 * A resume outside any pad body is refused statically; this is the
	 * other half, a pad entered by ordinary control flow. The bare return
	 * the JIT emits is right only for the bpf_throw() walker.
	 */
	if (!state->unwinding) {
		verbose(env,
	      ...
	      struct bpf_log_attr *attr_log)
{
	...
	/* Two successors, plus the landing pad edge a covered call adds. */
	env->succ = bpf_iarray_realloc(NULL, 3);
	if (!env->succ)
		goto skip_full_check;
