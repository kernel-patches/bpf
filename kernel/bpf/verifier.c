...
static bool verifier_inlines_kfunc_call(struct bpf_verifier_env *env, s32 imm)
{
	...
	if (is_bpf_wq_set_callback_impl_kfunc(desc->func_id))
		return true;

	if (desc->func_id == special_kfunc_list[KF_bpf_obj_new_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_percpu_obj_new_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_obj_drop_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_percpu_obj_drop_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_refcount_acquire_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_list_push_back_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_list_push_front_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_rbtree_add_impl] ||
	    desc->func_id == special_kfunc_list[KF_bpf_cast_to_kern_ctx] ||
