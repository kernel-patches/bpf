			       ...
			       const struct bpf_verifier_state *src)
{
	...
	dst_state->scc_visit_count = src->scc_visit_count;
				  ...
				  struct bpf_verifier_state *st)
{
	...
	/* Assume that read and precision marks for a state are incomplete if: */
	return /* - state->insn_idx is inside an SCC */
	       scc_info &&
	       /* - some states inside this SCC form a state loop because of
		*   states_equal(... RANGE_WITHIN) returning true in is_state_visited;
		*/
	       scc_info->incomplete_read_marks &&
	       /* - SCC was entered by some parent of the state */
	       scc_info->visit_count == st->scc_visit_count &&
	       /* - SCC has other states yet to be explored originating from it */
	       scc_info->branches > 0;
}

static void set_incomplete_read_marks(struct bpf_verifier_env *env,
				      struct bpf_verifier_state *st)
{
	struct bpf_scc_info *scc_info;
	...
	scc_info = insn_scc(env, st->insn_idx);
	if (scc_info)
		scc_info->incomplete_read_marks = 1;
}

/* See comment for bpf_scc_info->branches */
...
/* See comment for bpf_scc_info->{branches,visit_count} */
static void scc_exit(struct bpf_verifier_env *env, struct bpf_verifier_state *st)
{
	struct bpf_scc_info *scc_info;
	...
	if (scc_info) {
		...
		scc_info->branches--;
		/* States are traversed in DFS order, meaning that
		 * if some state enters SCC its .branches would become zero
		 * only when all its child states are fully explored.
		 * This would also set scc_info->branches to zero.
		 * All verifier states with scc_info->scc_visit_count == scc_info->visit_count
		 * received all read and precision marks they would ever get.
		 * Allow identifying this in incomplete_read_marks()
		 * by incrementing current visit_count.
		 */
		if (scc_info->branches == 0) {
			scc_info->visit_count++;
			// TODO: scc_info->incomplete_read_marks = 0; ?
		}
	}
}
					   ...
					   struct bpf_verifier_state *st)
{
	...
	for (i = 0; i <= st->curframe; i++) {
		insn_idx = i == st->curframe
			   ? st->insn_idx
			   : st->frame[i + 1]->callsite;
			      ...
			      struct bpf_verifier_state *cur)
{
	...
	list_for_each(pos, head) {
		...
		if (scc_info &&
		    scc_info->incomplete_read_marks &&
		    scc_info->visit_count > sl->state.scc_visit_count)
			...
			if (is_iter_next_insn(env, insn_idx)) {
				if (states_equal(env, &sl->state, cur, RANGE_WITHIN)) {
					...
					if (iter_state->iter.state == BPF_ITER_STATE_ACTIVE) {
						set_incomplete_read_marks(env, &sl->state);
						goto hit;
					}
				}
				goto skip_inf_loop_check;
...
miss:
	...
	if (scc_info)
		new->scc_visit_count = scc_info->visit_count;
