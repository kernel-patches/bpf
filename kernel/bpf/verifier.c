					 ...
					 int insn_idx)
{
	...
	field = reg_find_field_offset(&caller->regs[BPF_REG_1], caller->regs[BPF_REG_1].var_off.value,
				      BPF_RB_ROOT);
					...
					int argno)
{
	...
	struct_same = btf_struct_ids_match(&env->log, reg_btf, reg_ref_id, reg->var_off.value, meta->btf, ref_id, strict_type_match);
