	...
	for (i = 0; i < insn_cnt;) {
		...
		if (BPF_CLASS(insn->code) == BPF_LDX &&
			...
			if (!uaddress_limit)
				goto next_insn;
#ifdef CONFIG_X86_64
			/* Check if the address is a userspace address */
			*patch++ = BPF_MOV64_REG(BPF_REG_AX, insn->src_reg);
			if (insn->off)
				*patch++ = BPF_ALU64_IMM(BPF_ADD, BPF_REG_AX, insn->off);
			*patch++ = BPF_ALU64_IMM(BPF_RSH, BPF_REG_AX, 32);
			*patch++ = BPF_JMP_IMM(BPF_JLE, BPF_REG_AX, uaddress_limit >> 32, insn->off ? 6 : 5);

			/* Check if the address is above VSYSCALL_ADDR */
			*patch++ = BPF_MOV64_REG(BPF_REG_AX, insn->src_reg);
			if (insn->off)
				*patch++ = BPF_ALU64_IMM(BPF_ADD, BPF_REG_AX, insn->off);
			*patch++ = BPF_ALU64_IMM(BPF_ARSH, BPF_REG_AX, 21);
			*patch++ = BPF_JMP_IMM(BPF_JGE, BPF_REG_AX, (s32)(VSYSCALL_ADDR >> 21), 2);
