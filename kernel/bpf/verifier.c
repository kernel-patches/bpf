 ...
 * Assign an SCC number to each instruction in env->insn_aux[*].scc.
...
static int compute_scc(struct bpf_verifier_env *env)
{
	 ...
	 * - If 'low[v] == pre[v]', loops containing 'v' have beeb explored,
	 *   and 'v' can be considered the root of some SCC.
	 * - Because of the 'pre[u] < pre[v]' property, 'u' appears
	 *   earlier than 'v' in the 'dfs' array.
	 *
