...
static int bpf_compute_subprog_ret_regs(struct bpf_verifier_env *env)
{
	...
	for (subprog = 1; subprog < env->subprog_cnt; subprog++) {
		 ...
		 * type filter here. BTF that no compiler would emit can declare
		 * a return type that validation rejects, an array or a float
		 * say, which btf_resolve_size() would happily size at 16 bytes.
		 */
