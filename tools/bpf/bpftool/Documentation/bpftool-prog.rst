...
bpftool prog profile *PROG* [duration *DURATION*] *METRICs*
    ...
    program run. If any selected metric was not scheduled on a CPU, values for
    all metrics exclude that CPU so that ratios use a consistent CPU set. In
    JSON output, **run_cnt** is the total number of runs recorded by the
    profiler and **run_cnt_valid** is the number of runs included in metric
    values.
