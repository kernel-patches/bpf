...
struct profile_perf_event_opts {
	int map_fd;
	int group_fd;
	__u32 map_key;
};
...
static int
profile_open_perf_event(int mid, int cpu,
			const struct profile_perf_event_opts *opts)
{
	struct perf_event_attr attr = metrics[mid].attr;
	bool group_leader = opts->group_fd < 0;
	int pmu_fd;
	int err;
	...
	if (bpf_map_update_elem(opts->map_fd, &opts->map_key, &pmu_fd,
				BPF_ANY)) {
		err = -errno;
		close(pmu_fd);
		return err;
...
static int profile_open_perf_events(struct profiler_bpf *obj)
{
	...
	struct profile_perf_event_opts opts;
	unsigned int cpu, m;
	int pmu_fd;
	int err;
	...
	opts.map_fd = bpf_map__fd(obj->maps.events);
	for (cpu = 0; cpu < num_cpu; cpu++) {
		opts.group_fd = -1;
		opts.map_key = cpu;
		for (m = 0; m < ARRAY_SIZE(metrics); m++) {
			...
			pmu_fd = profile_open_perf_event(m, cpu, &opts);
			if (pmu_fd == -ENODEV && opts.group_fd < 0)
				break;
			if (pmu_fd < 0) {
			...
			if (opts.group_fd < 0)
				opts.group_fd = pmu_fd;
			opts.map_key += num_cpu;
		}
		if (opts.group_fd < 0)
			continue;
		if (ioctl(opts.group_fd, PERF_EVENT_IOC_ENABLE,
			  PERF_IOC_FLAG_GROUP)) {
			err = -errno;
