...
static int prog_has_dynload_tag(struct bpf_object *obj, struct bpf_program *prog)
{
	...
	for (i = 1; i < n; i++) {
		...
		if (!btf_is_func(fn_t) || btf_func_linkage(fn_t) != BTF_FUNC_GLOBAL) {
			pr_warn("prog '%s': " DYNLOAD_DECL_TAG " decl tag must be applied to a global function\n",
				prog->name);
			return -EINVAL;
		}
	...
	for (i = 0; i < obj->nr_programs; i++) {
		...
		if (prog->load_type != BPF_PROG_LOAD_TYPE_AUTO) {
			pr_debug("prog '%s': skipped loading, load type is %s\n", prog->name,
				 prog->load_type == BPF_PROG_LOAD_TYPE_DYNAMIC ? "dynamic" : "disabled");
			continue;
		}
...
static int bpf_object_init_progs(struct bpf_object *obj, const struct bpf_object_open_opts *opts)
{
	...
	bpf_object__for_each_program(prog, obj) {
		...
		if (err > 0) {
			...
			if (err) {
				pr_warn("prog '%s': failed to apply " DYNLOAD_DECL_TAG " decl tag: %s\n",
					prog->name, errstr(err));
				return err;
			}
		}
