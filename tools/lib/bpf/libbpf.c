...
retry_create:
	...
	if (pfd < 0) {
	   ...
	   /* If attachment fails with EADDRNOTAVAIL (or EINVAL in legacy mode) and we used non NULL
	    * func_name for attachment, try to find function address in /proc/kallsyms and retry
	    * using KADDR instead of ambiguous symbol as this error points to duplicate symbol in kallsyms.
	    * Legacy tracefs API converts EADDRNOTAVAIL to EINVAL, so we need to check for both.
		*/
		if ((err == -EADDRNOTAVAIL || (legacy && err == -EINVAL)) && optional_func_name) {
			pr_debug("Trying to find in /proc/kallsyms using absolute address for %s\n", func_name);

			/* Find kernel symbol address using kallsyms parser */
			kaddr_ctx.func_name = func_name;
			kaddr_ctx.kaddr = 0;  /* Explicitly initialize to 0 */

		   /* Drop the function symbol and override the offset to be the desired
			* function KADDR. This will avoid kallsyms validation for duplicate symbols in the kernel.
			* See comment regarding attr.config2 in perf_event_open_probe
			* and relevant kernel code in perf_kprobe_init and create_local_trace_kprobe.
			*/
			...
			if (err)
			{
				pr_warn("failed to obtain address for kernel symbol %s in /proc/kallsyms\n", func_name);
				goto err_out;
			}

			pr_debug("Retrying attachment to %s using kaddr 0x%llx\n",
