...
static struct btf *btf_new_empty(struct btf_new_opts *opts)
{
	...
	if (add_layout) {
		...
		btf->layout = btf->raw_data + hdr->hdr_len;
		memcpy(btf->layout, layouts, sizeof(layouts));
...
static int btf_dedup_compact_types(struct btf_dedup *d)
{
	...
	if (d->btf->layout)
		d->btf->hdr.layout_off = d->btf->hdr.type_len;
	d->btf->hdr.str_off = d->btf->hdr.type_len + d->btf->hdr.layout_len;
	d->btf->raw_size = d->btf->hdr.hdr_len + d->btf->hdr.type_len + d->btf->hdr.layout_len +
			   d->btf->hdr.str_len;
	return 0;
}
