...
static long btf_hash_enum(struct btf_type *t)
{
	...
	/* don't hash vlen and enum members and size to support enum fwd resolving */
	h = hash_combine(0, t->name_off);
	return h;
}
...
static bool btf_compat_enum(struct btf_type *t1, struct btf_type *t2)
{
	 ...
	 * - skip comparing size to allow enum forward declarations (size == 4)
	 *   to be compatible with enum64 full declarations (size == 8);
