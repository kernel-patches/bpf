...
static int btf_relocate_map_distilled_base(struct btf_relocate *r)
{
	...
	for (id = 1; id < r->nr_base_types; id++) {
		struct btf_name_info *dist_name_info, *dist_name_info_prev = NULL;
		...
		for (dist_name_info = search_btf_name_size(&base_name_info, dist_base_info_sorted,
		     ...
		     dist_name_info && dist_name_info < dist_base_info_sorted_end &&
		     (!dist_name_info_prev || !cmp_btf_name_size(&base_name_info, dist_name_info));
		     dist_name_info++) {
			dist_name_info_prev = dist_name_info;
