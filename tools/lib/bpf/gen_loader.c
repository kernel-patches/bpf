			...
			struct bpf_prog_load_opts *load_attr, int prog_idx)
{
	...
	/* populate union bpf_attr with a pointer to cleanup_info, but only for
	 * a program that has a table: the attr now runs to the end of the
	 * union, and a kernel that predates cleanup_info takes the longer attr
	 * only while the tail it does not know reads as zero.
	 */
