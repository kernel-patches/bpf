...
int run_bpftool_command(char *args)
{
	...
	ret = snprintf(cmd, BPFTOOL_CMD_MAX_LEN, "bpftool %s > /dev/null 2>&1",
		       args);
	if (ret !=
	    strlen(args) + strlen("bpftool ") + strlen(" > /dev/null 2>&1")) {
...
int get_bpftool_command_output(char *args, char *output_buf, size_t output_max_len)
{
	...
	ret = snprintf(cmd, BPFTOOL_CMD_MAX_LEN, "bpftool %s", args);
	if (ret != strlen(args) + strlen("bpftool ")) {
		fprintf(stderr, "Failed to generate bpftool command");
		return 1;
	}
