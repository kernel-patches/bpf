...
/*
 * Takes no argument on purpose. A return value larger than 16 bytes is passed
 * back through a hidden pointer argument (sret), which shifts every declared
 * argument by one register. GCC then describes the first one with a DWARF
 * location list rather than a plain register, and pahole, which maps
 * parameters to registers positionally, drops BTF encoding of the whole
 * function. resolve_btfids then reports "no BTF func for kfunc" and leaves the
 * ID at 0, which makes register_btf_kfunc_id_set() fail at module init. With
 * no arguments there is nothing to misplace.
 */
