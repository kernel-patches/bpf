...
/*
 * A global function takes its arguments as BTF describes them, and GCC passes
 * an aggregate by invisible reference instead, handing the callee a pointer
 * where the verifier expects the halves of the aggregate. The tests below are
 * left to clang. A static callee is not held to BTF, so the one above stands.
 */
...
/*
 * A static callee can take a struct that does not fit in the argument
 * registers: the compiler splits it across the last register and a stack
 * argument, and the verifier tracks the two halves as the separate slots they
 * are. Stack arguments need a JIT that supports them.
 */
...
int aggregate_arg_spilled_struct_c_test(struct __sk_buff *skb)
{
	...
	/*
	 * Every argument has to be a runtime value: given constants, the
	 * compiler is free to rewrite the signature of a static function and
	 * the call would no longer match the prototype in BTF.
	 */
