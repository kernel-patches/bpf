...
int kptr_refcount_run_leak;
int kptr_refcount_run_check;

SEC("fentry/bpf_fentry_test1")
int BPF_PROG(cgroup_storage_refcount_leak)
{
	struct lock_map_value *v;
	struct task_struct *task;
	u32 refcnt;

	if (!kptr_refcount_run_leak)
		return 0;
...
SEC("fentry/bpf_fentry_test1")
int BPF_PROG(check_cgroup_storage_refcount)
{
	struct lock_map_value *v;
	struct task_struct *task;
	u32 refcnt;

	if (!kptr_refcount_run_check)
		return 0;

	task = bpf_get_current_task_btf();
	v = bpf_cgrp_storage_get(&cgrp_strg, task->cgroups->dfl_cgrp, 0, 0);
	if (!v)
		return 0;
