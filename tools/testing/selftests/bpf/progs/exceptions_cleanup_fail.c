...
/*
 * A record whose range holds no call that can unwind. Nothing marks a call
 * site, so no edge reaches the pad and the CFG walk finds it unreachable.
 * A compiler that is told bpf_preempt_enable() is nothrow does not emit the
