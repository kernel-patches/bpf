...
extern u64 bpf_cpu_cyc2ns(u64 cycles) __weak __ksym;
extern u64 bpf_get_cpu_cycles_counter(void) __weak __ksym;
...
{
	asm volatile(
	"call %[bpf_get_cpu_cycles_counter];"
	"exit"
	:
	: __imm(bpf_get_cpu_cycles_counter)
	: __clobber_all
	);
}
