...
struct xdp_attach_error_ctx {
	 ...
	 * the size of the data. The last 2 bytes is the offset from the start of
	 * the tracepoint struct where the data begins.
...
int tp__xdp__bpf_xdp_link_attach_failed(struct xdp_attach_error_ctx *ctx)
{
	...
	void *ptr = (void *)(__u64) ((void *) ctx + (__u16) ctx->msg);
	bpf_probe_read_kernel_str(&errmsg.msg, sizeof(errmsg.msg), ptr);
