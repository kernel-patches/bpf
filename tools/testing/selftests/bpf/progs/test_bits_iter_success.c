...
int BPF_PROG(cpumask_memalloc, struct bpf_iter_meta *meta, struct cgroup *cgrp)
{
	...
	/* We have to use a hard-coded number here to avoid possible failure on s390x */
...
/* To avoid possible failure on s390x, we have to duplicate it. */
SEC("iter.s/cgroup")
int BPF_PROG(cpumask_copy, struct bpf_iter_meta *meta, struct cgroup *cgrp)
{
	...
	bpf_for_each(bits, cpu, p->cpus_ptr, 16) {
		rq = (struct rq *)bpf_per_cpu_ptr(&runqueues, *cpu);
		if (!rq)
			break;
		nr_running += rq->nr_running;
