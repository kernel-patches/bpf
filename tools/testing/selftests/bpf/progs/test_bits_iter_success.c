...
int BPF_PROG(cpumask_iter, struct bpf_iter_meta *meta, struct cgroup *cgrp)
{
	...
	/* READ_ONCE to avoid failure on s390x */
	bpf_for_each(bits, cpu, p->cpus_ptr, *(volatile int *)&total_nr_cpus) {
...
int BPF_PROG(null_pointer, struct bpf_iter_meta *meta, struct cgroup *cgrp)
{
	...
	bpf_for_each(bits, cpu, NULL, *(volatile int *)&total_nr_cpus)
		err++;
	return 0;
}
...
int BPF_PROG(invalid_bits, struct bpf_iter_meta *meta, struct cgroup *cgrp)
{
	...
	if (err)
		err -= 8192 - *(volatile int *)&total_nr_cpus;
	bpf_task_release(p);
	return 0;
}
