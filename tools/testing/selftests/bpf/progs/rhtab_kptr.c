 ...
 *
 * The delete program checks that the element really disappeared, otherwise
...
{
	struct val_t val = { .magic = RHTAB_MAGIC };
	u32 key = 0;

	if (bpf_map_update_elem(&rhtab, &key, &val, BPF_ANY))
		return 1;
	return 0;
}
