 ...
 *	frame foo2:  holds an RCU read lock plus a second tracked resource, and
 *		     has two call sites that can unwind (the call to foo3, and
 *		     its own throw) sharing one pad.
 *	frame foo1v: a static subprogram returning void, so its caller has no
 *		     return value to preserve across the test the kernel
 *		     injects -- and could not preserve one if it wanted to,
 *		     because a void frame need not leave anything in r0 and
 *		     its caller inherits whatever it left. That is why the
 *		     kernel puts "r0 = 0" at the entry of a void subprogram
 *		     that may unwind. Its pad also ends in a jump to a resume
 *		     block placed after an unrelated block that ends in a
 *		     plain exit, the layout LLVM produces when it sinks cold
 *		     EH blocks, so classifying the pad by scanning forward
 *		     from it rather than by following control flow would take
 *		     it for a catch pad and drop the exception.
...
static __used __noinline __u64 foo3(__u64 x)
{
	...
	"2:"
		 ...
		 * never returns. Nothing branches here once the throw has been
		 * lowered, and the kernel sweeps it away; it is here to
		 * exercise that sweep.
		 */
		"goto 3f;"
	"4:"					/* landing pad */
		 ...
		 * cover the kernel having to leave something defined there:
		 * this pad is reached from a throw, and the bpf_exc_set() that
		 * replaces the throw returns void, so r0 would otherwise be
		 * scrubbed at the branch.
		 */
		"r7 = r0;"
...
/*
 * Drop glue: a subprogram called from a landing pad, which is to say with an
 * exception already in flight. It returns void and, as far as the verifier can
 * tell, could be a call worth instrumenting -- but it is called from somewhere
 * the exception that instrumentation tests for is legitimately already raised,
 * so the kernel has to leave the call alone. Instrument it and the injected
 * "carry on unwinding" return leaves the pad with the RCU read lock still
 * held, which the verifier reports and the program does not load.
 *
 * It deliberately cannot throw. A pad that calls something which can is
 * refused at load -- see pad_calls_thrower in progs/exceptions_cleanup_fail.c
 * -- because there is nowhere to put a second exception raised while one is
...
/*
 * Middle frame. Owns two resources and has two call sites an exception can
...
"6:"					/* landing pad, shared by both regions */
	...
	: [cookie]"i"(THROW_COOKIE), [ran_rcu]"i"(RAN_FOO2_RCU),
	  [ran_trk]"i"(RAN_FOO2_TRACKER), __imm_addr(pads_ran)
	: __clobber_all);
}

/*
 * A frame whose call the caller cannot test the return value of.
 *
 * It returns void, so the compiler need not leave anything useful in r0 on the
 * way out, and the caller has nothing to compare against the unwind sentinel
 * unless the kernel puts something there. It does: a void subprogram that may
 * unwind gets "r0 = 0" at its entry.
 *
 * Its landing pad also jumps forward to a shared resume block, with the
 * function's own exit block laid out in between. Anything that classified the
 * pad by walking the instruction stream would find that exit first and take
 * this for a catch pad, pop the exception, and lose it before it ever reaches
 * the boundary.
 */
...
/*
 * A void subprogram that may unwind, called on an ordinary path rather than
 * from a pad. It is static, so the verifier walks into it and its caller sees
 * whatever it happened to leave in r0 -- and it deliberately leaves nothing,
 * which is what a void function is entitled to do. Without the "r0 = 0" the
 * kernel puts at the entry of a void subprogram that may unwind, the caller's
 * comparison against the unwind sentinel reads an uninitialised register and
