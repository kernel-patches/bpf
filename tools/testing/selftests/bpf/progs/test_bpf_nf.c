...
int test_ct_zone_alloc_entry = -EINVAL;
int test_ct_zone_insert_entry = -EAFNOSUPPORT;
int test_ct_zone_succ_lookup = -ENOENT;
int test_enoent_bad_ct_zone_lookup = 0;
	...
	if (ct) {
		...
		if (ct_ins) {
			...
			if (ct_lk)
				...
				test_enoent_bad_ct_zone_lookup = opts_def.error;

			bpf_ct_release(ct_ins);
			test_ct_zone_insert_entry = 0;
		}
		test_ct_zone_alloc_entry = 0;
	}
}
