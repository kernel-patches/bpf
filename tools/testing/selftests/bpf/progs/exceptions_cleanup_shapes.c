...
#define RAN_NOUNWIND_REC	0x2000
 ...
 *
 * A cleanup table leaves code dead in two places, and this frame has both.
 * The continuation after a bpf_throw() and the tail after a pad's
 * bpf_unwind_resume() are each explored by bpf_check_cfg() as an ordinary
 * call's fall-through, and each dies later, when do_check() declines to walk
 * past the throw or the resume. The same dead code sweep removes both. This
 * frame is all of the shapes that has to get right, in one function:
 *
 ...
 *    bpf_insn_successors() but not to bpf_check_cfg();
 ...
 *
 * Nothing reads the interior of a region anyway. cleanup_mark_call_sites()
 * walks it for the calls an exception can unwind out of, and the table the
 * walker ends up searching is rebuilt one record per covered call. But a rule
 * against a region ending on a 16-byte instruction would turn a frontend
 * emitting one away, so here is one being emitted.
 */
...
/*
 * 13. A pad that works out of its own frame's stack.
 *
 * The shape every compiler-generated pad has: what the pad needs was put in
 * the frame before the call and the pad reloads it, because there are only
 * four callee-saved registers and a pad routinely calls out. A pad runs on
 * the bpf_throw() walker's stack with only its own frame's registers put
 * back, so a JIT that addresses the BPF frame through the stack pointer --
 * arm64 does, to keep the offsets positive -- has to address a pad's frame
 * some other way, or every one of these lands in the walker's frame instead.
 *
 * Both directions matter and both are here: the reload sees the frame, and
 * the store lands in the frame rather than on top of whatever the kernel has
 * below the pad.
 */
...
/*
 * 14. A region around a call the kernel knows cannot unwind.
 *
 * A frontend is free to emit one: a helper and an ordinary kfunc are both
 * nounwind here, so nothing in the range is a call site an exception can leave
 * by. No call site is marked, the pad is reachable from nothing, and the sweep
 * removes it. The program is otherwise ordinary and has to load.
 */
