...
int kfunc_call_test5(struct __sk_buff *skb)
{
	 ...
	 *
	 * All tests: multiply → truncate to parameter size → zero-extend in kfunc
	 */
 ...
 * This ensures consistent testing across all 4 ISA versions
 * (with/without ALU32, with/without v2/v3 instructions).
 */
SEC("tc")
