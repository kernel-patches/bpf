...
int bits_memalloc_non_aligned_bits(void)
{
	...
	/* If you want to iterate over 68 bits, you must split it into two steps */
	bpf_for_each(bits, bit, &data,  sizeof(u64))
		nr++;

#if defined(__TARGET_ARCH_s390)
	offset = sizeof(u64) - (68 - sizeof(u64) + 7) / 8;
#endif

	bpf_for_each(bits, bit, ((char *)&data) + sizeof(u64) + offset, 68 - sizeof(u64))
		nr++;
	return nr;
}
