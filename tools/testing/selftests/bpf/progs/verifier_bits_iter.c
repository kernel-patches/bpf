...
int BPF_PROG(no_destroy, struct bpf_iter_meta *meta, struct cgroup *cgrp)
{
	...
	bpf_iter_bits_new(&it, p->cpus_ptr, 8);
...
{
	u32 data = 0b101111100001000100101U;
	int nr = 0;
	int *bit;

	bpf_for_each(bits, bit, &data, 4)
		nr++;
	return nr;
}
...
__success __retval(20)
int bits_copy64(void)
{
	u64 data = 0xf0f0f0f0fUL;
	int nr = 0;
	int *bit;

	bpf_for_each(bits, bit, &data, 8)
		nr++;
	return nr;
}
...
int bits_memalloc(void)
{
	...
	bpf_for_each(bits, bit, &data, sizeof(data))
		nr++;
	return nr;
}
