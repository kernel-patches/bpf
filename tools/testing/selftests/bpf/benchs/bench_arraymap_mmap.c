...
static void *producer(void *input)
{
	while (true) {
		...
		addr = mmap(NULL, ctx.mmap_sz, PROT_READ | PROT_WRITE,
			    MAP_SHARED | MAP_POPULATE, ctx.map_fd, 0);
		if (addr == MAP_FAILED) {
			fprintf(stderr, "mmap failed: %d\n", -errno);
			exit(1);
		}
		/*
		 * Access one byte in every page so the measured cost includes
		 * touching the whole mapping, not just the mmap()/munmap()
		 * bookkeeping. The volatile access keeps the compiler from
		 * optimizing the read away.
		 */
