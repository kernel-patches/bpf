...
access_map() {
	...
	access_for_write "$name_cmd" "$map_name" "$bpftool_path" "$key_for_rw" "$write_should_succeed"
	access_for_deletion "$name_cmd" "$map_name" "$bpftool_path" "$key_to_del" "$write_should_succeed"
	iterate_map_elem "$name_cmd" "$map_name" "$bpftool_path" "$bpf_dir" "$bpf_iter_file_path"
}
...
test_map_creation_and_map_of_maps() {
	...
	if "$bpftool_path" map update name "$outer_map_name" key 2 0 0 0 value name "$inner_map_name" 2>/dev/null; then
...
test_map_access "$PROTECTED_MAP_NAME" "$BPFTOOL_PATH" "$BPF_DIR" "false" "$BPF_ITER_FILE_PATH"

# Test not protected map (write should succeed).
test_map_access "$NOT_PROTECTED_MAP_NAME" "$BPFTOOL_PATH" "$BPF_DIR" "true" "$BPF_ITER_FILE_PATH"
