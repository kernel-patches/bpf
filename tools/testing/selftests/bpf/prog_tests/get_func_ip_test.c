...
static void test_prog_array_call_get_func_ip_compat(void)
{
	...
	if (!ASSERT_OK(err, "update prog_array without get_func_ip"))
		goto cleanup;

	err = bpf_map_update_elem(map_fd, &key, &with_get_func_ip_fd, BPF_ANY);
	if (!ASSERT_ERR(err, "mixed call_get_func_ip update must fail"))
		goto cleanup;
	ASSERT_EQ(errno, EINVAL, "mixed call_get_func_ip update errno");

	close(map_fd);
	map_fd = bpf_map_create(BPF_MAP_TYPE_PROG_ARRAY, NULL, sizeof(key),
				sizeof(__u32), 1, NULL);
	if (!ASSERT_GE(map_fd, 0, "bpf_map_create_2"))
		goto cleanup;

	err = bpf_map_update_elem(map_fd, &key, &with_get_func_ip_fd, BPF_ANY);
	if (!ASSERT_OK(err, "update prog_array with get_func_ip"))
		goto cleanup;

	err = bpf_map_update_elem(map_fd, &key, &without_get_func_ip_fd, BPF_ANY);
	if (!ASSERT_ERR(err, "mixed reverse call_get_func_ip update must fail"))
		goto cleanup;
	ASSERT_EQ(errno, EINVAL, "mixed reverse call_get_func_ip update errno");
