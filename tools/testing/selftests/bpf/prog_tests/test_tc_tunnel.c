...
static int setup(void)
{
	...
	if (make_netns(CLIENT_NS) || make_netns(SERVER_NS))
		goto fail;

	nstoken = open_netns(CLIENT_NS);
	SYS(fail, "ip link add veth1 mtu 1500 netns %s address %s type veth "
	    "peer name veth2 mtu 1500 netns %s address %s",
	    CLIENT_NS, MAC_ADDR_VETH1, SERVER_NS, MAC_ADDR_VETH2);
