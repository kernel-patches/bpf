...
#ifndef LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS_EXECTIME
#define LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS_EXECTIME BIT(4)
#endif

#ifndef LANDLOCK_RESTRICT_SELF_EXECTIME
#define LANDLOCK_RESTRICT_SELF_EXECTIME BIT(5)
#endif
...
static const struct restrict_variant flag_variants[] = {
	...
	{
		.name = "flag_exectime_rejected",
		.previous_domain = PREV_DOMAIN_NONE,
		.restrict_flags = LANDLOCK_RESTRICT_SELF_EXECTIME,
		.expected_restrict_ret = -EINVAL,
		.expect_enforced = false,
	},
};
...
static int create_path_ruleset(const char *path, const struct operation_case *op)
{
	struct landlock_ruleset_attr attr = {
		.handled_access_fs = op->handled_access_fs,
	};
	...
	err = add_path_rule(ruleset_fd, path, op->allowed_access_fs);
	if (err) {
		close(ruleset_fd);
		return err;
	}
...
static int run_exec_attempt(struct landlock_kfuncs *skel,
			    ...
			    bool enable_bpf, int ruleset_fd,
			    int userspace_ruleset_fd,
			    __u32 userspace_restrict_flags,
			    int *child_status)
{
	...
	if (child == 0) {
		...
		err = create_and_apply_previous_domain(
			env, variant->previous_domain, op->handled_access_fs);
		if (err)
			_exit(-err);

		if (userspace_ruleset_fd >= 0) {
			if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0))
				_exit(errno);
			if (landlock_restrict_self(userspace_ruleset_fd,
						  userspace_restrict_flags))
				_exit(errno);
			close(userspace_ruleset_fd);
		}
	 ...
	 const struct restrict_variant *variant, const char *subtest_name)
{
	...
	err = run_exec_attempt(skel, env, op, hook, variant, false, -1, -1, 0,
			       &child_status);
...
static void run_userspace_pending_after_bpf_case(struct landlock_kfuncs *skel,
					 const struct landlock_test_env *env,
					 const struct hook_variant *hook,
					 const char *subtest_name)
{
	const struct operation_case *const op = &operation_cases[1];
	const struct restrict_variant variant = {
		.name = "pending_bpf_before_userspace",
		.previous_domain = PREV_DOMAIN_NONE,
		.restrict_flags = 0,
		.expect_enforced = true,
	};
	...
	err = seed_operation_state(env, op);
	if (!ASSERT_OK(err, "seed_userspace_only"))
		return;

	userspace_ruleset_fd = create_path_ruleset(env->restricted_file, op);
	if (!ASSERT_GE(userspace_ruleset_fd, 0, "create_userspace_ruleset"))
		return;

	err = run_exec_attempt(skel, env, op, hook, &variant, false, -1,
			       userspace_ruleset_fd,
			       LANDLOCK_RESTRICT_SELF_EXECTIME, &child_status);
	close(userspace_ruleset_fd);
	if (!ASSERT_OK(err, "userspace_only_exec"))
		return;
	...
	err = seed_operation_state(env, op);
	if (!ASSERT_OK(err, "seed_combined"))
		return;

	bpf_ruleset_fd = create_exec_ruleset(env, op);
	if (!ASSERT_GE(bpf_ruleset_fd, 0, "create_bpf_ruleset"))
		return;

	userspace_ruleset_fd = create_path_ruleset(env->restricted_file, op);
	if (!ASSERT_GE(userspace_ruleset_fd, 0, "create_userspace_ruleset")) {
		close(bpf_ruleset_fd);
		return;
	}

	err = run_exec_attempt(skel, env, op, hook, &variant, true,
			       bpf_ruleset_fd, userspace_ruleset_fd,
			       LANDLOCK_RESTRICT_SELF_EXECTIME, &child_status);
	close(userspace_ruleset_fd);
	close(bpf_ruleset_fd);
	if (!ASSERT_OK(err, "combined_exec"))
		return;

	assert_operation_outcome(env, op, false, child_status);
	assert_bpf_state(skel, hook, true, 0);
	ASSERT_OK(delete_ruleset_map_elem(skel), "delete_ruleset_map_elem");
}
...
void test_landlock_kfuncs(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(hook_variants); i++) {
...

		{
			...
			ASSERT_LT(snprintf(name, sizeof(name),
					   "%s/write_file/pending_bpf_before_userspace",
					   hook_variants[i].name),
				  (int)sizeof(name), "subtest_name_len");
			run_userspace_pending_after_bpf_case(skel, &env,
						    &hook_variants[i], name);
		}
	}
