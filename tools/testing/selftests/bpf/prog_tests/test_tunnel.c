			   ...
			   const char *type, const char *opt)
{
	...
	SYS(fail, "ip netns exec at_ns0 sysctl -wq net.ipv6.conf.%s.accept_dad=0", dev0);
	SYS(fail, "sysctl -wq net.ipv6.conf.%s.accept_dad=0", dev1);
...
static void dev0_ipv4_check(void)
{
	/* ping from root namespace test */
	ASSERT_OK(test_ping(AF_INET, IP4_ADDR_TUNL_DEV0), "ping TUNL_DEV0");
}

static void dev1_ipv4_check(void)
{
	...
	ASSERT_OK(test_ping(AF_INET, IP4_ADDR_TUNL_DEV1), "ping TUNL_DEV1");
	close_netns(nstoken);
}

static void ipv4_check(void)
{
	dev0_ipv4_check();
	dev1_ipv4_check();
}

static void veth0_ipv6_check(void)
{
	ASSERT_OK(test_ping(AF_INET6, IP6_ADDR_VETH0), "ping6 veth0");
}

static void dev0_ipv6_check(void)
{
	ASSERT_OK(test_ping(AF_INET6, IP6_ADDR_TUNL_DEV0), "ping6 TUNL_DEV0");
}

static void dev1_ipv6_check(void)
{
	struct nstoken *nstoken;
	...
	ASSERT_OK(test_ping(AF_INET, IP6_ADDR_TUNL_DEV1), "ping6 TUNL_DEV1");
	close_netns(nstoken);
}
