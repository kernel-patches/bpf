...
static int add_ipip_tunnel(enum ipip_encap encap)
{
	...
	if (type && ipproto) {
		...
		SYS(fail, "ip -n at_ns0 link add dev %s type ipip local %s "
			  "remote %s encap %s encap-sport auto "
			  "encap-dport 5555 noencap-csum",
		    IPIP_TUNL_DEV0, IP4_ADDR_VETH0, IP4_ADDR1_VETH1, type);
	} else {
		SYS(fail, "ip -n at_ns0 link add dev %s type ipip local %s "
			  "remote %s",
