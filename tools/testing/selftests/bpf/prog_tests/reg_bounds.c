...
static int total_case_cnt = 0, case_cnt = 0;
...
static int verify_case(enum num_t init_t, enum num_t cond_t, struct range x, struct range y)
{
	...
	for (sub.op = MIN_OP; sub.op <= MAX_OP; sub.op++) {
		...
		case_cnt++;
		if ((case_cnt % 10000) == 0) {
			double progress = (case_cnt + 0.0) / total_case_cnt;
			u64 elapsed_ns = get_time_ns() - start_ns;
			double remain_ns = elapsed_ns / progress;
...
static void gen_vals(void)
{
	...
	for (i = 1, j = 0; i < cnt; i++) {
		if (usubvals[j] == usubvals[i])
			continue;
		j++;
		usubvals[j] = usubvals[i];
	}
	subval_cnt = j + 1;

	for (i = 0; i < subval_cnt; i++)
		ssubvals[i] = usubvals[i];
...
static int gen_ranges(void)
{
	...
	for (i = 0; i < val_cnt; i++) {
		for (j = i; j < val_cnt; j++) {
			uranges[range_cnt] = range(U64, uvals[i], uvals[j]);
			sranges[range_cnt] = range(S64, svals[i], svals[j]);
			range_cnt++;
		}
	}
	...
	usubranges = calloc(cnt, sizeof(*usubranges));
	if (!ASSERT_OK_PTR(usubranges, "usubranges_calloc"))
		return -EINVAL;
	ssubranges = calloc(cnt, sizeof(*ssubranges));
	if (!ASSERT_OK_PTR(ssubranges, "ssubranges_calloc"))
		return -EINVAL;

	for (i = 0; i < subval_cnt; i++) {
		for (j = i; j < subval_cnt; j++) {
			usubranges[subrange_cnt] = range(U32, usubvals[i], usubvals[j]);
			ssubranges[subrange_cnt] = range(S32, ssubvals[i], ssubvals[j]);
			subrange_cnt++;
		}
	}
...
static void validate_gen_ranges_vs_consts(void)
{
	...
	for (cond_t = MIN_T; cond_t <= MAX_T; cond_t++) {
		uconst = range(U32, usubvals[i], usubvals[i]);
		sconst = range(S32, ssubvals[i], ssubvals[i]);
...
void test_reg_bounds_gen(void)
{
	...
	total_case_cnt = (MAX_OP - MIN_OP + 1) * (MAX_T - MIN_T + 1) * (
		(4 * range_cnt * val_cnt) +
