...
/*
 * Compiler (gcc) can ignore __naked attribute and place endbr64 at the
 * begining of the function, so let's just scan few more bytes to make
 * sure we have nop5 addres/offset.
 */
...
static void test_uprobe_regs_equal(bool retprobe)
{
	...
	offset = get_nop5_offset(&uprobe_regs_trigger, NULL);
	if (!ASSERT_GE(offset, 0, "get_nop5_offset"))
		return;
	...
	for (i = 0; i < cnt; i++) {
		...
		case offsetof(struct pt_regs, ss):
			break;
		case offsetof(struct pt_regs, rax):
			/* uprobe does not see return value */
			if (!retprobe)
				break;
		default:
...
static void test_uprobe_legacy(void)
{
	...
	offset = get_nop5_offset(&uprobe_test, &addr);
	if (!ASSERT_GE(offset, 0, "get_nop5_offset"))
		return;
	...
	check(skel, link, uprobe_test, addr);
...
static void *worker_attach(void *arg)
{
	...
	offset = get_nop5_offset(&uprobe_test, NULL);
	if (!ASSERT_GE(offset, 0, "get_nop5_offset"))
		return NULL;
...
static void __test_uprobe_syscall(void)
{
	...
	if (test__start_subtest("uprobe_regs_equal"))
		test_uprobe_regs_equal(false);
	if (test__start_subtest("uprobe_regs_change"))
		test_uprobe_regs_change();
}
#else
static void __test_uprobe_syscall(void)
