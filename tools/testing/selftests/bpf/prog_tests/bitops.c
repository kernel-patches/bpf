...
{
	(void)s;
	return (__u64)__builtin_ctzll(x);
}
...
static void test_bitops_case(const char *prog_name, bool skip_x0)
{
...
#define RUN_BITOPS_CASE(_bitops, _prog, _skip_x0)			\
	do {								\
		size_t i;						\
									\
		for (i = 0; i < ARRAY_SIZE(cases); i++) {		\
			if ((_skip_x0) && cases[i].x == 0)		\
				continue;				\
			cases[i].exp = _bitops(cases[i].x, cases[i].s);	\
		}							\
		test_bitops_case(_prog, _skip_x0);			\
	} while (0)

static void test_clz64(void)
{
	RUN_BITOPS_CASE(clz64, "bitops_clz64", false);
}

static void test_ctz64(void)
{
	RUN_BITOPS_CASE(ctz64, "bitops_ctz64", false);
}

static void test_ffs64(void)
{
	RUN_BITOPS_CASE(ffs64, "bitops_ffs64", true);
}

static void test_fls64(void)
{
	RUN_BITOPS_CASE(fls64, "bitops_fls64", false);
}

static void test_bitrev64(void)
{
	RUN_BITOPS_CASE(bitrev64, "bitops_bitrev", false);
}

static void test_popcnt64(void)
{
	RUN_BITOPS_CASE(popcnt64, "bitops_popcnt", false);
}

static void test_rol64(void)
{
	RUN_BITOPS_CASE(rol64, "bitops_rol64", false);
}

static void test_ror64(void)
{
	RUN_BITOPS_CASE(ror64, "bitops_ror64", false);
}
