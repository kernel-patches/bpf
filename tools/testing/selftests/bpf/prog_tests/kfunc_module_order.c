...
void test_kfunc_module_order(void)
{
	...
	err = load_module("bpf_test_modorder_x.ko", false);
	if (!ASSERT_OK(err, "load bpf_test_modorder_x.ko"))
		return;

	err = load_module("bpf_test_modorder_y.ko", false);
...
exit_mods:
	unload_module("bpf_test_modorder_y", false);
exit_modx:
	unload_module("bpf_test_modorder_x", false);
}
