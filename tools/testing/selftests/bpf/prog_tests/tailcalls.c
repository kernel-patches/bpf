...
{
	struct bpf_object *obj = NULL, *freplace_obj = NULL;
	int err, map_fd, prog_fd, data_fd, i, val;
	struct bpf_program *prog, *freplace_prog;
	struct bpf_link *freplace_link = NULL;
	struct bpf_map *prog_array, *data_map;
	int freplace_prog_fd;
	char buff[128] = {};
	...
	err = bpf_prog_test_load("tc_bpf2bpf.bpf.o", BPF_PROG_TYPE_SCHED_CLS,
				 &obj, &prog_fd);
	if (!ASSERT_OK(err, "load obj"))
		goto out;
	...
	freplace_obj = bpf_object__open_file("tailcall_bpf2bpf_hierarchy_freplace.bpf.o",
					     NULL);
	if (!ASSERT_OK_PTR(freplace_obj, "open freplace_obj file"))
		goto out;

	freplace_prog = bpf_object__find_program_by_name(freplace_obj, "entry");
	if (!ASSERT_OK_PTR(freplace_prog, "find freplace prog"))
		goto out;
	...
	if (!ASSERT_OK(err, "load freplace_obj"))
		goto out;

	freplace_prog_fd = bpf_program__fd(freplace_prog);
	if (!ASSERT_GE(freplace_prog_fd, 0, "freplace_prog_fd"))
		goto out;
	...
	prog_array = bpf_object__find_map_by_name(freplace_obj, "jmp_table");
	if (!ASSERT_OK_PTR(prog_array, "find jmp_table"))
		goto out;

	fprintf(stdout, "before map_fd\n");

	map_fd = bpf_map__fd(prog_array);
	if (!ASSERT_GE(map_fd, 0, "map_fd"))
		goto out;

	fprintf(stdout, "before update jmp_table\n");

	i = 0;
	err = bpf_map_update_elem(map_fd, &i, &freplace_prog_fd, BPF_ANY);
	if (!ASSERT_OK(err, "update jmp_table"))
		goto out;

	fprintf(stdout, "before find data_map\n");

	data_map = bpf_object__find_map_by_name(freplace_obj, ".bss");
	if (!ASSERT_FALSE(!data_map || !bpf_map__is_internal(data_map),
			  "find data_map"))
		goto out;

	data_fd = bpf_map__fd(data_map);
	if (!ASSERT_GE(data_fd, 0, "data_fd"))
		goto out;

	fprintf(stdout, "before run tc prog\n");
	fprintf(stdout, "before run tc prog2\n");
	fprintf(stdout, "before run tc prog3\n");

	err = bpf_prog_test_run_opts(prog_fd, &topts);
	ASSERT_OK(err, "tc prog run");
	ASSERT_EQ(topts.retval, 1, "tc prog run retval");
	...
	ASSERT_EQ(topts.retval, 1, "tc prog run retval again");

	i = 0;
	err = bpf_map_lookup_elem(data_fd, &i, &val);
	ASSERT_OK(err, "tailcall count again");
	ASSERT_EQ(val, 35, "tailcall count again");
...
{
	test_tailcall_bpf2bpf_hierarchy_freplace(false);
}
...
{
	test_tailcall_bpf2bpf_hierarchy_freplace(true);
}
