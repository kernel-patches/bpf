...
static int sys_and_log(const char *fmt, ...)
{
	va_list args;
	char cmd[1024];
	char logbuf[2048];
	...
	out = popen(cmd, "r");
	if (!ASSERT_OK_PTR(out, cmd))
		return -1;

	fgets(logbuf, sizeof(logbuf), out);
	status = pclose(out);

	if (!ASSERT_OK(status, "pclose")) {
		log_err("status %d, %s\n", status, logbuf);
		return -1;
	}
...
static int setup_redirect_target(const char *target_dev, bool need_mac)
{
	...
	if (sys_and_log("ip route add %s dev link_err encap bpf xmit obj %s sec %s 2>&1",
