...
{
	int map_fd, *keys, value_size, cpu, i, j, nr_cpus, err;
	...
	skel = percpu_array_flag__open();
	if (!ASSERT_OK_PTR(skel, "percpu_array_flag__open"))
		return;

	bpf_program__set_autoload(skel->progs.test_percpu_array, false);

	err = percpu_array_flag__load(skel);
	if (!ASSERT_OK(err, "percpu_array_flag__load"))
		goto out;
