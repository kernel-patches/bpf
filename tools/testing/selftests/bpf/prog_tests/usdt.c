...
static void subtest_optimized_attach(void)
{
	...
	if (!ASSERT_OK_PTR(addr_1, "find_nop1"))
		return;

	addr_2 = find_instr(usdt_2, nop1_nop5_combo, 6);
	if (!ASSERT_OK_PTR(addr_2, "find_nop1_nop5_combo"))
		return;
	 ...
	 * The usdt probe does not have extra data in elf note record,
	 * so we expect the probe to land on the first nop without being
	 * optimized.
	 */
	 ...
	 * of nop1,nop5 combo. The extra data in the elf note record and
