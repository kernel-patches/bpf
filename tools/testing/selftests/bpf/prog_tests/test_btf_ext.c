...
static void subtest_line_func_info(void)
{
	 ...
	 */
	if (!ASSERT_MEMEQ(libbpf_line_info, line_info, libbbpf_line_info_cnt * sizeof(*line_info),
			  "line_info")) {
		FILE *f;
		char buf[4096];

		SYS(out, "sudo ./bpftool prog dump xlated name global_func > dump.txt");
		f = fopen("dump.txt", "rt");
		while (fgets(buf, sizeof(buf), f)) {
			fprintf(stderr, "%s\n", buf);
		}
		SYS(out, "objdump -D test_btf_ext.bpf.o");
	}
	if (!ASSERT_MEMEQ(libbpf_func_info, func_info, libbbpf_func_info_cnt * sizeof(*func_info),
		"func_info")) {
		SYS(out, "sudo ./bpftool prog dump xlated name global_func");
		SYS(out, "objdump -D test_btf_ext.bpf.o");
	}
out:
