...
static void subtest_thp_policy(void)
{
	...
	ops_link = bpf_map__attach_struct_ops(skel->maps.khugepaged);
...
static void subtest_thp_fork(void)
{
	...
	ops_link = bpf_map__attach_struct_ops(skel->maps.fork);
	if (!ASSERT_OK_PTR(ops_link, "attach struct_ops"))
		return;
...
static void subtest_thp_policy_update(void)
{
	...
	old_link = bpf_map__attach_struct_ops(skel->maps.fork);
	if (!ASSERT_OK_PTR(old_link, "attach_old_link"))
		return;

	new_link = bpf_map__attach_struct_ops(skel->maps.khugepaged);
	if (!ASSERT_NULL(new_link, "attach_new_link"))
		goto destory_old;
	ASSERT_EQ(errno, EBUSY, "attach_new_link");

	err = bpf_link__update_map(old_link, skel->maps.khugepaged);
