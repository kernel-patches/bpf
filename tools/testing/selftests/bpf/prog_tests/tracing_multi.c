...
static void test_fentry_after_multi(void)
{
	...
	if (!ASSERT_OK(err, "test_run"))
		goto cleanup;
	if (!ASSERT_EQ(skel->bss->test_result_fentry_1, 1, "multi_fentry"))
		goto cleanup;
	if (!ASSERT_EQ(skel->bss->test_result_fentry, 1, "fentry"))
		goto cleanup;
	...
	if (!ASSERT_OK(err, "test_run_multi"))
		goto cleanup;
	if (!ASSERT_EQ(skel->bss->test_result_fentry_1, 2, "multi_fentry_only"))
		goto cleanup;
	if (!ASSERT_EQ(skel->bss->test_result_fentry, 1, "fentry_detached"))
		goto cleanup;
