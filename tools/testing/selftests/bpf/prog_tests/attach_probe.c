...
static void test_kprobe_write_ctx_prog_array_compat(void)
{
	...
	if (!ASSERT_OK(err, "update prog_array with read-only kprobe"))
		goto cleanup;

	err = bpf_map_update_elem(map_fd, &key, &write_prog_fd, BPF_ANY);
	if (!ASSERT_ERR(err, "mixed kprobe_write_ctx update must fail"))
		goto cleanup;
	ASSERT_EQ(errno, EINVAL, "mixed kprobe_write_ctx update errno");
	...
	err = bpf_map_update_elem(map_fd, &key, &write_prog_fd, BPF_ANY);
	if (!ASSERT_OK(err, "update prog_array with write-ctx kprobe"))
		goto cleanup;

	err = bpf_map_update_elem(map_fd, &key, &read_prog_fd, BPF_ANY);
	if (!ASSERT_ERR(err, "mixed reverse kprobe_write_ctx update must fail"))
		goto cleanup;
	ASSERT_EQ(errno, EINVAL, "mixed reverse kprobe_write_ctx update errno");
