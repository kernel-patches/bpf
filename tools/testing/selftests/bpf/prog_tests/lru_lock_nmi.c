 ...
 *
 * Post-stress: per-CPU pinned threads refill each CPU's quota and the
 * main thread verifies every inserted key is still present. A
 * permanently stranded node would force eviction of a just-inserted
 * key on its CPU, surfacing as a missing lookup.
 *
 ...
 * we stay correct on hosts with nr_cpus >= CPU_SETSIZE (default 1024).
 */
static int pin_to_cpu(int cpu, int nr_cpus)
{
	cpu_set_t *cs;
	size_t cs_size;
	int err;

	cs = CPU_ALLOC(nr_cpus);
	if (!cs)
		return -ENOMEM;
	cs_size = CPU_ALLOC_SIZE(nr_cpus);
...
static void *hammer_thread(void *p)
{
	...
	pin_to_cpu(a->cpu, a->nr_cpus);

	/* Per-thread seed so rand_r() doesn't share state across CPUs. */
 ...
 * quota of keys (pinned). For common-LRU the partitioning is harmless;
 * for the per-CPU variants it respects each CPU's local pool so that on
 * a leak-free kernel no insert needs to evict. After refill, lookup
 * every key we inserted - a stranded node on any CPU's pool would have
 * forced eviction of a just-inserted key on that CPU, surfacing here as
 * a missing lookup.
 */
static int drain_then_verify_capacity(int map_fd, int nr_cpus)
{
	...
	for (i = 0; i < nr_cpus; i++) {
		...
		if (pthread_create(&threads[nthreads], NULL, refill_thread,
				   &args[i]) == 0)
			nthreads++;
	}
			...
			const char *name)
{
	...
	ASSERT_OK(drain_then_verify_capacity(map_fd, nr_cpus),
		  "drain_then_verify_capacity");
