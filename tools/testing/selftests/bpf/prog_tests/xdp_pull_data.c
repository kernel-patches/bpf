...
int find_xdp_data_hard_end(struct test_xdp_pull_data *skel, int frame_sz)
{
	...
	prog_fd = bpf_program__fd(skel->progs.xdp_find_data_hard_end);
...
static void test_xdp_pull_data_basic(void)
{
	...
	if (find_xdp_data_hard_end(skel, pg_sz))
		goto out;
