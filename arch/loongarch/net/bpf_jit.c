...
#define LOONGARCH_TAIL_CALL_OFFSET_NBYTES	(6 * 4)
...
static void __build_epilogue(struct jit_ctx *ctx, bool is_tail_call)
{
	...
	if (!is_tail_call) {
		...
		/* goto *t3; */
		emit_insn(ctx, jirl, LOONGARCH_GPR_ZERO, LOONGARCH_GPR_T3, 0);
	}
}
...
static int emit_bpf_tail_call(struct jit_ctx *ctx, u32 map_index, int insn)
{
...
static int build_insn(const struct bpf_insn *insn, struct jit_ctx *ctx, bool extra_pass)
{
	...
	case BPF_JMP | BPF_TAIL_CALL:
		if (emit_bpf_tail_call(ctx, imm, i) < 0)
			return -EINVAL;
		break;
