...
{
	emit(A64_MOVZ(1, A64_R(10), func_meta, 0), ctx);
			      ...
			      u32 flags)
{
	...
	int cookie_off, cookie_cnt, cookie_idx;
	u64 func_meta;
	...
	cookie_idx = (bargs_off - cookie_off) / 8;
	for (i = 0; i < fentry->nr_links; i++) {
		if (bpf_link_prog_session_cookie(fentry->links[i])) {
			u64 meta = func_meta | (cookie_idx << BPF_TRAMP_COOKIE_INDEX_SHIFT);
