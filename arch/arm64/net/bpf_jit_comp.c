			  ...
			  bool is_exception_cb, u64 arena_vm_start)
{
	...
	/* If freplace's target prog is main prog, it has to make R26 as
	 * tail_call_cnt_ptr, and then initialize tail_call_cnt via
	 * tail_call_cnt_ptr.
	 */
	if (is_main_prog && is_ext) {
		...
		emit(A64_ADD_I(1, ptr, A64_SP, 32), ctx);
...
#define jmp_offset (out_offset - (cur_offset))
	 ...
	 * prevent re-initializing tail_call_cnt.
	 */
	emit(A64_MOVZ(1, r0, 0, 0), ctx);
