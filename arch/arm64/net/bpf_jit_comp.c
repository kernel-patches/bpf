...
#define PROLOGUE_OFFSET (BTI_INSNS + 2 + PAC_INSNS + 12)
...
static void invoke_bpf_prog(struct jit_ctx *ctx, struct bpf_tramp_link *l,
			    ...
			    bool save_ret)
{
	int ctx_tc_run_ctx_off = run_ctx_off - offsetof(struct bpf_tramp_run_ctx, tc_run_ctx);
	int ctx_tcc_off = run_ctx_off - offsetof(struct bpf_tramp_run_ctx, tail_call_cnt);
	int cookie_off = offsetof(struct bpf_tramp_run_ctx, bpf_cookie);
	int ctx_tcc_ptr_off = ctx_tcc_off - 8;
	...
	if (p->aux->use_tail_call_run_ctx) {
		...
		emit(A64_STR64I(r1, fp, -ctx_tc_run_ctx_off), ctx);
		/* Update rdi as tcc_ptr */
		emit(A64_SUB_I(1, r1, fp, ctx_tcc_off), ctx);
		/* Clear bpf_tail_call_cnt */
		emit(A64_MOV(1, tmp, 0), ctx);
		emit(A64_STR32I(tmp, r1, 0), ctx);
		/* Cache tcc_ptr */
		emit(A64_STR64I(r1, fp, -ctx_tcc_ptr_off), ctx);
		/* Update r1 as tail call run ctx */
		emit(A64_SUB_I(1, r1, fp, ctx_tc_run_ctx_off), ctx);
	}
