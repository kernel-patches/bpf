...
static int emit_bpf_tail_call(struct jit_ctx *ctx)
{
	...
	/*
	 * The throw-site spill sits between the program stack and the
	 * callee-saved area, so a tail call has to release it for the same
	 * reason build_epilogue() does: otherwise pop_callee_regs() below
	 * reads the spill instead of the saved registers, and the tail call
	 * target inherits an SP that is short by the whole area.
	 */
	if (ctx->throw_spill)
