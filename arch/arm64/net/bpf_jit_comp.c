...
static int calc_arg_align(const struct btf_func_model *m, int i)
{
	/* Assume that arguments follow their natural alignment (ie: no
	 * attribute like __packed or __aligned).
	 */
...
static int calc_arg_aux(const struct btf_func_model *m,
			 struct arg_aux *a)
{
	int stack_slots, nregs, slots, ret, i;

	for (i = 0; i < m->nr_args; i++) {
		ret = calc_arg_align(m, i);
		if (ret < 0)
			return ret;
		a->arg_align[i] = ret;
	}
	...
	for (a->ostack_for_args = 0, a->bstack_for_args = 0;
	     i < m->nr_args; i++) {
		/* BTF is not enough to know exacly about alignment
		 * and packing customizations, so just deny functions with
		 * structures passed on stack
		 */
