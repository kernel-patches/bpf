...
/*
 * Where a landing pad's frame is anchored. Generated code addresses the BPF
 * frame through the stack pointer, which inside a pad belongs to the
 * bpf_throw() walker rather than to the frame being cleaned up after, so a pad
 * computes the equivalent of that stack pointer from BPF r10 on entry and uses
 * it instead. x24 holds it: bpf2a64[] maps nothing to x24, so no other
 * generated code touches it, the prologue spills it like the rest, and being
 * callee-saved it survives every call the pad makes. x23 carries the pad's
 * return address for the same reasons.
 */
		      ...
		      struct jit_ctx *ctx, bool extra_pass)
{
	...
	/*
	 * A landing pad is entered with its own frame's registers put back but
	 * on the bpf_throw() walker's stack, so the stack pointer the frame is
	 * normally addressed through is the wrong one. Anchor the frame on
	 * A64_CLEANUP_FP here, once per pad, and address it off that for as
	 * long as the pad runs; see the comment on A64_CLEANUP_FP.
	 */
...
emit_cond_jmp:
	...
	case BPF_JMP | BPF_CALL:
	{
		...
		if (bpf_cleanup_insn_is_throw(ctx->prog, insn - ctx->prog->insnsi)) {
			/*
			 * Spill where the bpf_throw() walker looks, in
			 * push_callee_regs() order; the gap at 32 is x23/x24,
			 * which nothing generated touches. A64_STR64I scales
			 * an unsigned offset, so tmp is the area's bottom.
			 */
