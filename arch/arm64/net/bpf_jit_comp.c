...
{
	/* Structures exact alignment needs can not be deduced only from
	 * function model, especially if they have special
	 * packing/alignment attributes
	 */
	if (m->arg_flags[i] & BTF_FMODEL_STRUCT_ARG)
		return 0;

	/* For any other supported fundamental type, alignment can be
	 * deduced from size
	 */
			 ...
			 struct arg_aux *a)
{
	...
	for (a->ostack_for_args = 0, a->bstack_for_args = 0;
		...
		a->ostack_for_args = round_up(
			a->ostack_for_args + stack_slots * 8, a->arg_align[i]);
	}
