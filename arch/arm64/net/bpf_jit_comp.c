...
{
	return slot != BPF_JIT_ARG_TMP && slot >= arm64_arg_abi.nr_arg_regs;
}

static bool a64_arg_in_reg(u8 slot)
{
	return slot != BPF_JIT_ARG_TMP && slot < arm64_arg_abi.nr_arg_regs;
}
...
static void emit_kfunc_arg_moves(struct jit_ctx *ctx, const struct btf_func_model *fm)
{
	...
	const u8 stage = bpf2a64[TMP_REG_1];	/* carries one move off the stack */
	const u8 held = bpf2a64[TMP_REG_2];	/* carries a cycle across its moves */
	u32 i, n;
	...
	for (i = 0; i < n; i++) {
		...
		if (src == BPF_JIT_ARG_TMP) {
			reg = held;
		} else if (a64_arg_on_stack(src)) {
			reg = a64_arg_in_reg(dst) ? dst : stage;
		...
		if (dst == BPF_JIT_ARG_TMP)
			emit(A64_MOV(1, held, reg), ctx);
		else if (a64_arg_on_stack(dst))
