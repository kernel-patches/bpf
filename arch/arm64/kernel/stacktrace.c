...
arch_bpf_unwind_ra_consume_entry(const struct kunwind_state *state, void *cookie)
{
	...
	/*
	 * A consumer that stops the walk still gets to redirect the frame it
	 * stopped on -- the last frame an unwind reaches is the one it has to
	 * deliver to. Store what it left in @ra before honouring the stop.
	 */
