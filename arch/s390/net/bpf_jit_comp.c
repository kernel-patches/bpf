		      ...
		      u64 func_meta, int cookie_off)
{
	...
	for (i = 0; i < tl->nr_links; i++) {
		if (tl->links[i]->link.prog->call_session_cookie) {
			u64 meta = func_meta |
				((u64)cur_cookie << BPF_TRAMP_COOKIE_INDEX_SHIFT);
					 ...
					 void *func_addr)
{
	...
	if (fsession_cnt) {
		...
		EMIT4_IMM(0xa7090000, REG_0, 0);
		for (i = 0; i < cookie_cnt; i++)
			EMIT6_DISP_LH(0xe3000000, 0x0024, REG_0, REG_0,
				      REG_15, cookie_off + 8 * i);
		/* clear the return value to make sure fentry always gets 0 */
		EMIT6_DISP_LH(0xe3000000, 0x0024, REG_0, REG_0, REG_15, tjit->retval_off);
	}
