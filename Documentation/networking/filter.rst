...
When CONFIG_BPF_JIT_ALWAYS_ON is enabled, bpf_jit_enable is initialized to 1
and setting a zero value will return failure. This does still allow setting
bpf_jit_enable to 2 for dumping the final JIT image into the kernel log, but
is discouraged unless introspection using bpftool (under tools/bpf/bpftool/)
