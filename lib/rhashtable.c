 ...
 * positioned at the beginning.  For rhltables, the iterator is positioned at
 * the first entry in @key's chain.
 *
				...
				__must_hold(RCU)
{
	...
	iter->p = he;
	if (ht->rhlist)
		iter->list = container_of(he, struct rhlist_head, rhead);
}
