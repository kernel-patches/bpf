 ...
 *
 * %LANDLOCK_RESTRICT_SELF_TSYNC is mutually exclusive with
 * %LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS_EXECTIME and
 * %LANDLOCK_RESTRICT_SELF_EXECTIME.
 *
		...
		flags)
{
	...
	if (!new_cred)
		return -ENOMEM;
	err = landlock_restrict_cred(new_cred, ruleset, flags, true);
	if (err) {
