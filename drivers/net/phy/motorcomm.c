...
static int yt8824_phy_package_probe_once(struct phy_device *phydev)
{
	...
	if (res < 0) {
		phydev_err(phydev, "missing phy-mode in PHY package node.\n");
		return res;
	}

	if (priv->interface_mode != PHY_INTERFACE_MODE_INTERNAL &&
	    priv->interface_mode != PHY_INTERFACE_MODE_10G_QXGMII) {
		phydev_err(phydev, "unsupported phy-mode %s\n",
			   phy_modes(priv->interface_mode));
		return -EINVAL;
	}
