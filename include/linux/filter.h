...
/*
 * The same walk, with @ra naming the stack slot @ip was read from. Writing
 * there changes where that frame returns to, which is how an exception
 * cleanup landing pad is dispatched. Only the arches whose
 * bpf_jit_supports_cleanup_pads() says yes provide it.
 */
