...
struct bpf_prog_aux {
	/*
	 * Native address of this (sub)program's epilogue. An unwind sends a
	 * frame that has no landing pad here, so it returns at once and its
	 * epilogue restores its caller's registers on the way. Recorded for
	 * every program: a frame on an unwind path need not carry a cleanup
	 * table of its own.
	 */
	u64 epilogue_ip;
	atomic64_t refcnt;
