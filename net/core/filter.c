	   ...
	   __be32 *, to, u32, to_size, __wsum, seed)
{
	...
	if (from_size && to_size)
		return from32to16(csum_sub(csum_partial(to, to_size, 0),
					   csum_partial(from, from_size, 0)));
	if (to_size)
		return from32to16(csum_partial(to, to_size, seed));

	if (from_size)
		return from32to16(csum_sub(0, csum_partial(from, from_size, seed)));
