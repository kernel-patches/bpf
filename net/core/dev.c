...
static void bpf_xdp_link_log(const union bpf_attr *attr, struct netlink_ext_ack *extack)
{
	struct bpf_generic_log log;
	const struct bpf_generic_user_log *ulog = &attr->link_create.log;

	BPF_GENERIC_VLOG_WRITE(&log, ulog, extack->_msg);
}
