 ...
 *		   -EINVAL - opts->ct_zone_id or opts->ct_zone_dir set when
			...
			u32 timeout)
{
	...
	if (!opts || !bpf_tuple || opts->reserved[0] || opts->reserved[1] || opts->reserved[2])
	...
	if (opts_len == NF_BPF_CT_OPTS_SZ) {
		...
		if (opts->ct_zone_id || opts->ct_zone_dir)
			/* opts->ct_zone_id and opts->ct_zone_dir must not be set */
			return ERR_PTR(-EINVAL);
